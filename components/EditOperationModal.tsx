...
export default function EditOperationModal({op, onClose, onSave}: EditOperationModalProps){
  ...
  // Hora apertura MT5 — inputs separados HH:MM (formato 24h) para mínima carga cognitiva
  const initHoraMt5=op.hora_apertura_mt5?new Date(op.hora_apertura_mt5):null;
  const[hh,setHH]=useState(initHoraMt5?String(initHoraMt5.getHours()).padStart(2,"0"):"");
  const[mm,setMM]=useState(initHoraMt5?String(initHoraMt5.getMinutes()).padStart(2,"0"):"");
  ...
  const handleSave=async()=>{
    ...
    // Construir ISO desde HH:MM (asume HOY como fecha)
    let horaAperturaMT5:string|null=null;
    const hhNum=parseInt(hh,10);
    const mmNum=parseInt(mm,10);
    if(!isNaN(hhNum)&&!isNaN(mmNum)&&hhNum>=0&&hhNum<=23&&mmNum>=0&&mmNum<=59){
      const today=new Date();
      today.setHours(hhNum,mmNum,0,0);
      horaAperturaMT5=today.toISOString();
    }
    setSaving(true);
        ...
        <div style={{marginBottom:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <input type="number" min={0} max={23} value={hh}
              ...
              placeholder="HH"
              style={{...inp,width:60,textAlign:"center",fontFamily:MONO,fontSize:16,fontWeight:700,padding:"6px 4px"}}/>
            <span style={{fontFamily:MONO,fontSize:18,fontWeight:700,color:T.muted}}>:</span>
            ...
            }} style={{padding:"5px 10px",background:T.s3,border:`1px solid ${T.border}`,
              borderRadius:5,color:T.muted,fontFamily:SANS,fontSize:10,cursor:"pointer"}}>
              Ahora
            </button>
            {(hh||mm)&&(
              <button type="button" onClick={()=>{setHH("");setMM("");}}
                style={{padding:"5px 8px",background:"none",border:"none",
                  color:T.muted,fontFamily:SANS,fontSize:11,cursor:"pointer"}} title="Borrar">
                ✕
              </button>
            )}
          </div>
          <div style={{fontSize:7,color:T.dim,fontFamily:SANS,marginTop:3}}>
            Formato 24h (igual que MT5). Asume hoy. Vacío = usa hora de registro.
          </div>
        </div>
        <div style={{marginBottom:8}}>
