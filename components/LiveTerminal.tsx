...
function SessionBanner({session}:{session:Session}){
  ...
  const m:Record<Session,{label:string;sub:string;color:string;bg:string;border:string}>={
    ...
    LDN:{label:"LDN",sub:"Mercado abierto · liquidez alta (+1)",color:T.wait,bg:T.warnBg,border:T.warnBorder},
    NY:{label:"NY",sub:"Mercado abierto · liquidez alta (+1)",color:T.up,bg:T.upBg,border:T.upBorder},
    CLOSED:{label:"MERCADO",sub:"Mercado abierto · liquidez baja (−2)",color:T.muted,bg:T.s1,border:T.border},
...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  useEffect(()=>{
    async function load(){
      ...
      try{
        ...
        const v=getLiveVerdict(
          ...
          d1SigLocal,sessionLevels,slPct,tpPct,
    ...
    return()=>clearInterval(iv);
  },[htfTf,ltfTf,slPct,tpPct,session,hasNews]);  // Bug 6.1 — slPct/tpPct en deps: cambiarlos recalcula al instante. session/hasNews: gates de Capa 2
