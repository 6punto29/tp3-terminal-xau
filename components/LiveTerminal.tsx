...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  // Detectar permiso al cargar
  useEffect(()=>{
    if(typeof window==="undefined"||!("Notification" in window))return;
    setNotifPerm(Notification.permission);
  },[]);

  // Pedir permiso desde botón
  ...
  // Sticky verdict compacto (1 línea) usado SOLO en mobile <700px.
  // Si querés cambiar el formato del sticky, es esta línea — el resto
  // (VerdictCard completo) sigue intacto dentro del tab "Veredicto".
  const mobileVerdictText = signal
    ? (signal.verdict==="ESPERAR"
        ? `ESPERAR · ${signalScore}/10 · ${structureLabel}`
        : `${signal.verdict} ${signal.strength} · ${signalScore}/10 · ${structureLabel}`)
    : "Cargando...";
  const mobileVerdictColor = signal && signal.verdict!=="ESPERAR" ? T.up : T.muted;
  ...
  return(
    <>
      <style>{`
        ...
        /* Mobile <700px: sticky verdict compacto arriba (~40px) + bottom nav (~56px) +
           un panel visible por tab. Auditoría responsive 03/06/26 — patrón híbrido. */
        @media(max-width:700px){
          .tp3-mobile-verdict{display:block;position:fixed;top:0;left:0;right:0;z-index:100;background:${T.bg};border-bottom:1px solid ${T.border};padding:10px 12px;font-family:${MONO};font-size:11px;font-weight:700;text-align:center;letter-spacing:0.3px;}
          .tp3-mobile-nav{display:grid;grid-template-columns:1fr 1fr 1fr;position:fixed;bottom:0;left:0;right:0;z-index:100;background:${T.bg};border-top:1px solid ${T.border};}
          ...
          .tp3-root{grid-template-columns:1fr;height:100vh;padding-top:42px;padding-bottom:56px;box-sizing:border-box;}
          .tp3-sidebar,.tp3-main,.tp3-right{grid-column:1;grid-row:1;overflow-y:auto;height:auto;max-height:calc(100vh - 98px);}
          ...
          .tp3-root[data-tab="analisis"] .tp3-right{display:none!important;}
      ...
      <div className="tp3-mobile-verdict" style={{color:mobileVerdictColor}}>
        {mobileVerdictText}
      </div>
      ...
      <div className="tp3-root" data-tab={mobileTab} style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-right"
          ...
          {/* Notificaciones push — solo visible si no están activadas */}
          {notifPerm!=="granted"&&(
            <div style={{padding:"6px 10px",marginBottom:6,borderRadius:8,
              background:notifPerm==="denied"?`${T.down}12`:`${T.accent}12`,
              border:`1px solid ${notifPerm==="denied"?T.down:T.accent}40`,
              display:"flex",alignItems:"center",justifyContent:"space-between",gap:6}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:SANS,fontSize:9,fontWeight:700,
                  color:notifPerm==="denied"?T.down:T.accent}}>
                  {notifPerm==="denied"?"🔕 Notificaciones bloqueadas":"🔔 Activar alertas de señal"}
                </div>
                <div style={{fontFamily:SANS,fontSize:8,color:T.muted,marginTop:1}}>
                  {notifPerm==="denied"
                    ?"Habilitalas desde la config del navegador"
                    :"Recibí avisos cuando aparezca ENTRAR"}
                </div>
              </div>
              {notifPerm==="default"&&(
          ...
          {notifPerm==="granted"&&(
            <div style={{padding:"4px 10px",marginBottom:6,borderRadius:6,
              background:T.s2,border:`1px solid ${T.border}`,
              fontFamily:SANS,fontSize:8,color:T.muted,
              display:"flex",alignItems:"center",gap:4}}>
              <span style={{color:T.up}}>🔔</span>
              <span>Alertas activas · cooldown 90s</span>
            </div>
          )}
          <Checklist
