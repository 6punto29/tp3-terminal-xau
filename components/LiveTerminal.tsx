...
function MobileBottomNav({active,onChange,bottomOffset}:{active:MobileTab;onChange:(t:MobileTab)=>void;bottomOffset:number}){
  ...
  return(
    <nav className="tp3-mobile-bottom-nav" style={{
      position:"fixed",bottom:`${bottomOffset}px`,left:0,right:0,zIndex:100,
...
export default function LiveTerminal({userId,price,connected,notifPerm,notifEnabled}:{userId:string;price:number;connected:boolean;notifPerm:"default"|"granted"|"denied";notifEnabled:boolean}){
  ...
  // ── DIAGNÓSTICO TEMPORAL — investigando bug visual del bottom nav en PWA iOS ─
  // El probe mide el valor real de env(safe-area-inset-bottom) en runtime.
  // Si el valor cambia dinámicamente (ej: 34 ↔ 0) cuando hay scroll, confirma
  // el bug iOS y aplicamos max() en una segunda etapa. REMOVER después de validar.
  const safeAreaProbeRef = useRef<HTMLDivElement>(null);
  const[envDebug,setEnvDebug] = useState({insetB:0, vh:0, ch:0});

  // ── FIX iOS PWA bottom nav — compensa cambio dinámico de window.innerHeight ─
  // En iOS PWA standalone, window.innerHeight (vh) crece durante el scroll
  // mientras document.documentElement.clientHeight (ch) se mantiene estable.
  // Eso desplaza position:fixed;bottom:0 hacia abajo. Calculamos vh-ch y lo
  // pasamos al nav como offset para que se mantenga pegado al layout real.
  ...
  // ── DIAGNÓSTICO TEMPORAL — mide env(safe-area-inset-bottom) cada 500ms ────
  // Lee el alto computado del probe (que tiene height:env(safe-area-inset-bottom))
  // y también vh/ch del viewport. Si insetB cambia entre estados, el bug está
  // confirmado. REMOVER después de validar.
  // Adicionalmente: calcula vh-ch para compensar el cambio dinámico de
  // window.innerHeight en iOS PWA y pasarlo al bottom nav.
  useEffect(()=>{
    const measure=()=>{
      const h=safeAreaProbeRef.current?.getBoundingClientRect().height ?? 0;
      ...
      // Diferencia: si vh > ch, el nav debe subir esa cantidad para mantenerse
      // pegado al layout viewport real (ch estable) y no al innerHeight dinámico.
      // Math.max(0, ...) protege contra cualquier caso donde vh < ch.
      setNavBottomOffset(Math.max(0, vh - ch));
    };
    measure();
    // Polling cada 500ms como backup
    const id=setInterval(measure,500);
    // Listeners para responder rápido a cambios del viewport
  ...
  return(
    <>
      ...
      <MobileBottomNav active={mobileTab} onChange={handleMobileTab} bottomOffset={navBottomOffset}/>

      {/* ── DIAGNÓSTICO TEMPORAL — REMOVER después de validar el bug visual PWA ─
          Probe invisible que tiene altura = env(safe-area-inset-bottom).
          El useEffect mide su altura real cada 500ms y la muestra en el display. */}
      <div ref={safeAreaProbeRef} aria-hidden="true" style={{
        position:"fixed", bottom:0, left:0, width:0,
        height:"env(safe-area-inset-bottom)",
        visibility:"hidden", pointerEvents:"none",
      }}/>

      {/* Display visible solo en mobile, posicionado abajo del topbar.
          NO usa env() en su posicionamiento para no enmascarar el bug. */}
      {isMobile && (
        <div style={{
          position:"fixed",
          top:"calc(env(safe-area-inset-top) + 48px)",
          left:8, right:8, zIndex:101,
          ...
          debug · inset-b={envDebug.insetB}px · vh={envDebug.vh} · ch={envDebug.ch}
        </div>
      )}
    </>
  );
