...
function MT5Block({signal,score,price,slPct,tpPct}:MT5BlockProps){
  ...
  // Lotaje correcto: capital × 1% riesgo ÷ SL en dólares
  ...
  const lotaje = slPoints>0?calcLotSize(capital,1,slPoints):0;
  ...
  return(
    <Card style={{marginBottom:4,padding:"8px 10px",border:`1px solid ${color}40`,background:`${color}08`}}>
      ...
      {[
        ...
        {lbl:"LOTAJE", val:lotaje.toFixed(2),color:T.accent,hint:`$${capital.toLocaleString()} × 1%`},
  ...
  const save=async()=>{
    ...
    try{
      ...
      const res=await fetch("/api/operations",{method:"POST",headers:{"Content-Type":"application/json","x-user-id":userId},
        body:JSON.stringify({fecha,direccion:dir,precio_entrada:eNum,sl:slNum,tp:tpNum,lotaje:lotSize>0?lotSize:null})});
          ...
          <div key={op.id}>
            ...
            {op.resultado!=null&&(
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"4px 10px",
                ...
                {/* % cuenta */}
                {op.pnl!=null&&(()=>{
                  // NOTA: % calculado sobre capital ACTUAL (no el del momento de la op)
                  // Para % histórico real, guardar capital_moment en DB
                  const cap=parseFloat((typeof window!=="undefined"&&localStorage.getItem("tp3_capital"))||"10000")||10000;
                  const pct=(op.pnl/cap)*100;
                  return(<div>
                    <div style={{fontSize:7,color:T.muted,letterSpacing:"0.05em",textTransform:"uppercase"}}>% cuenta</div>
                    <div style={{fontFamily:MONO,fontSize:12,fontWeight:700,color:pnlColor}}>
                      {pct>=0?"+":""}{pct.toFixed(2)}%
                    </div>
                  </div>);
    ...
    <>
      ...
      <div className="tp3-root" style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-main"
          ...
          <MT5Block signal={signal} score={signalScore} price={price} slPct={slPct} tpPct={tpPct}/>
          <Card>
