...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  const mtfRows=[
    ...
    {key:"mtf", tf:htfTf==="4h"?"1H":"15M", rol:"Confirmación", sig:(signal?.mtf??"WAIT") as MTFSig},
  ...
  return(
    <>
      <style>{`
        .tp3-root{display:grid;grid-template-columns:300px 1fr 340px;grid-template-rows:1fr;height:100%;overflow:hidden;}
