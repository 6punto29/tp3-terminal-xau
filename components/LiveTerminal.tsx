...
import type { LiquidityLevel } from "@/lib/engine/signals";
...
//   · 35%+ tiempo + R:R OK + override tendencia → 🟢 ENTRADA (cartel "tendencia ultra fuerte")
...
// D1 + 4H + HTF + MTF todos a favor del trade + estructura confirmada
// (LH/LL para SHORT, HH/HL para LONG). Si la estructura se rompe a NEUTRAL,
// el override se desactiva y el semáforo cae a 🟡 TARDÍA pasado el 35%.
function EntryValidityIndicator({
  ...
  // Usamos snap.sl / snap.tp (congelados al momento de detectar la señal) y NO
  // los levels del engine en vivo. Razón: el engine puede dejar de retornar
  ...
  //
  // Cambio 01/06/26 v6: se agregó structureConfirmed para evitar que el override
  // siguiera activo cuando la estructura intradía se rompía (caso real observado
  // 31/05-01/06 con persistencia 6h+ a pesar de estructura NEUTRAL).
...
function Checklist({session,signal,price,signalScore,hasNews,onToggleNews}:ChecklistProps){
  ...
  const states:RowState[]=[
    ...
    (ema200!=null&&price>0&&Math.abs(price-ema200)/ema200>0.002&&
      ((signal?.htf==="UP"&&price>ema200)||(signal?.htf==="DOWN"&&price<ema200))) ? "ok":"fail",
    (signal!=null&&signal.htf!=="WAIT"&&signal.htf===signal.mtf) ? "ok":"fail",
    (rsi!=null&&rsi>=30&&rsi<=70) ? "ok":"fail",
    (!hasNews) ? "ok":"fail",
