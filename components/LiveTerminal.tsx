...
//
// FIX TOKEN EXPIRY (08/06/26): el JWT default de Supabase dura 60 min.
// El SDK tiene auto-refresh interno (setTimeout) que falla cuando el tab
// queda en background o sin interacción mucho tiempo (timers estrangulados
// por el navegador). Síntoma observado: 401s acumulativos en el tracker
// de Shadow Trading (corre cada 5 min via setInterval del motor).
//
// Defensa: chequear expires_at proactivamente y forzar refreshSession()
// si el token está vencido o por vencer en <60s. Sin requests extra en
// el caso común (token vigente). Si el refresh_token también vence
// (~30 días sin entrar), retorna headers vacíos → API 401 limpio →
// onAuthStateChange redirige al /login.
...
// COUNTDOWN DE CIERRE DE SESIÓN ACTIVA (08/06/26):
// Devuelve cuántos minutos faltan para que cierre la sesión LDN o NY actual.
// Retorna null si la sesión NO está activa (CLOSED/WEEKEND ya tienen su
// countdown propio vía getNextSession).
//   LDN activa → minutos hasta ldnOpen + SESSION_LEN (5h)
//   NY  activa → minutos hasta nyOpen + SESSION_LEN (5h)
function getCurrentSessionMinsRemaining(session:Session):number|null{
  if(session!=="LDN"&&session!=="NY")return null;
  const now=new Date();
  const u=now.getUTCHours()*60+now.getUTCMinutes();
  const closeAt=(session==="LDN"?ldnOpenUTC():nyOpenUTC())+SESSION_LEN;
  const remaining=closeAt-u;
  // Defensivo: si por algún edge case el cálculo da ≤0 o no positivo,
  // devolvemos null para no mostrar "Cierra en 0m" o negativos.
...
function Checklist({session,signal,price,signalScore,hasNews,onToggleNews}:ChecklistProps){
  ...
  return(
    <Card style={{marginBottom:4,padding:"6px 10px"}}>
...
function SessionBanner({session}:{session:Session}){
  ...
  // COUNTDOWN DE CIERRE (08/06/26): minutos restantes hasta que cierre la
  // sesión activa (LDN o NY). null cuando session es CLOSED/WEEKEND — ahí ya
  // existe `next` con el countdown de la PRÓXIMA sesión, son complementarios.
  const[closesIn,setClosesIn]=useState<number|null>(null);
  useEffect(()=>{
    setClock(getColTime());
    if(session==="CLOSED"||session==="WEEKEND")setNext(getNextSession());
    if(session==="LDN"||session==="NY")setClosesIn(getCurrentSessionMinsRemaining(session));
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <div>
          ...
          {(session==="LDN"||session==="NY")&&closesIn!=null&&(
            <div style={{fontFamily:MONO,fontSize:9,color:s.color,marginTop:2}}>
              ⏱ Cierra en {fmtCountdown(closesIn)}
            </div>
          )}
        </div>
      </div>
...
function CopyTerminalBtn(p:CopyTerminalProps){
  ...
  const copy=()=>{
    ...
    const txt=[
      ...
      // S3 (11/06/26): línea "📊 CUENTA" removida. Las métricas del motor viven en
      // la pestaña Cuenta (analítico), no en el clipboard del terminal (operativo).
      // Coherente con el rediseño 2 columnas + anti-patrón "trading the P&L".
      `━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    ].filter(l=>l!=="").join("\n");
...
type MobileTab = "veredicto" | "analisis";
...
// ── Dedup key para signals_emitted (regla #24) ─────────────────────────────
// Mismo patrón que shadowDedupKey: bucket horario UTC. 1 captura por dirección
// por hora calendario UTC. Independiente del htfTf actual para que rehidratar
// funcione consistentemente sin importar la config del usuario.
function signalEmittedDedupKey(
  direction: "LONG" | "SHORT",
  atMs: number,
): string {
  const d = new Date(atMs);
  d.setUTCMinutes(0, 0, 0);
  return `signal-${direction}-${d.getTime()}`;
}

// ── Singletons al nivel del módulo (fix bug remountaje 10/06/26) ─────────────
// Sobreviven a remontajes del componente. Cuando el usuario cambia entre
// pestañas (Terminal → Cuenta → Terminal), LiveTerminal se desmonta y vuelve
// a montarse. Los useRef originales vivían DENTRO del componente y se
// reiniciaban a Set vacío / "ESPERAR" / 0 en cada remontaje. Esto causaba:
//   (a) Duplicados de señales en el mismo bucket horario UTC (shadowSeenKeys,
//       signalEmittedSeenKeys) — observado el 10/06/26: 4 SHORTs en bucket
//       01:00 UTC con datos casi idénticos, también Eventos 2-3 de Shadow
//       el 07/06/26 a 15:13-15:14 UTC con datos al céntimo.
//   (b) Re-disparo del push notification al volver a Terminal con una señal
//       activa (lastVerdictRef se reiniciaba a "ESPERAR" → veía "transición
//       ESPERAR→ENTRAR" falsa) — observado por Steven el 09/06/26.
//
// Solución: lifteados a nivel módulo. El useRef dentro del componente apunta
// a la misma instancia entre remontajes. Compatible con admin-only (1 user).
// Para multi-user real (futuro): se complementa con Single-Active-Session
// (pendiente apuntado en memoria para el sistema de usuarios).
const __shadowSeenKeys                        = new Set<string>();
const __signalEmittedSeenKeys                 = new Set<string>();
const __lastVerdictRef:  { current: Verdict } = { current: "ESPERAR" };
const __cooldownLongRef:  { current: number } = { current: 0 };
const __cooldownShortRef: { current: number } = { current: 0 };

// ── Vida útil de una señal antes de marcarla EXPIRED (24h, igual a Shadow) ──
const SIGNAL_EMITTED_EXPIRY_MS = 24 * 60 * 60 * 1000;

// ── Aviso de señal por vencer: cuánto antes de las 24h se muestra (2h) ──
const SIGNAL_EXPIRY_WARN_MS = 2 * 60 * 60 * 1000;

// Señal OPEN que se acerca al vencimiento por TIEMPO (sin tocar TP/SL).
// Alimenta el banner de aviso arriba de la columna Veredicto.
interface ExpiringSignal {
  id:         string;
  direction:  "LONG" | "SHORT";
  entryPrice: number;
  createdAt:  number;
...
// ── Outcome checker para signals_emitted ───────────────────────────────────
// Determina si una señal abierta debe cerrarse (WIN/LOSS/EXPIRED) en base al
// precio actual y el timestamp de expiración. Espejo de checkShadowOutcome
// adaptado a una sola TP por señal (en Shadow hay 4 perfiles).
//
// Reglas:
// · LONG  → WIN si livePrice >= tp, LOSS si livePrice <= sl
// · SHORT → WIN si livePrice <= tp, LOSS si livePrice >= sl
// · EXPIRED si pasó SIGNAL_EMITTED_EXPIRY_MS desde createdAt
// · Si toca TP/SL y SL al mismo tiempo (gap raro), prioriza SL (conservador)
type SignalEmittedOutcomeStatus = "OPEN" | "WIN" | "LOSS" | "EXPIRED";
interface SignalEmittedOutcome {
  status:    SignalEmittedOutcomeStatus;
  hitPrice:  number | null;
  closedAt:  number | null;
}

function checkSignalEmittedOutcome(
  params: {
    entryPrice: number;
    tpPrice:    number;
    slPrice:    number;
  ...
  livePrice: number,
): SignalEmittedOutcome {
  const now    = Date.now();
  const isLong = params.direction === "LONG";

  // Prioridad SL (más conservador en gaps que rompen ambos niveles a la vez)
  if (isLong && livePrice <= params.slPrice) {
    return { status: "LOSS", hitPrice: params.slPrice, closedAt: now };
  }
  if (!isLong && livePrice >= params.slPrice) {
    return { status: "LOSS", hitPrice: params.slPrice, closedAt: now };
  }
  ...
  // TP
  if (isLong && livePrice >= params.tpPrice) {
    return { status: "WIN", hitPrice: params.tpPrice, closedAt: now };
  }
  if (!isLong && livePrice <= params.tpPrice) {
    return { status: "WIN", hitPrice: params.tpPrice, closedAt: now };
  }
  ...
  // Expiración
  const expiresAt = params.createdAt + SIGNAL_EMITTED_EXPIRY_MS;
  if (now >= expiresAt) {
    return { status: "EXPIRED", hitPrice: livePrice, closedAt: now };
  }

  return { status: "OPEN", hitPrice: null, closedAt: null };
}

// ── Outcome checker contra klines 1m (patch retroactivo del 10/06/26) ──────
// Resuelve el bug del tracker en-vivo: comparar solo livePrice cada 5min
// pierde extremos intermedios. Por ejemplo el 09/06/26 ~20:51 UTC el precio
// tocó $4187 (por debajo del TP $4191.71 de 2 señales SHORT) y rebotó arriba
// del TP antes del próximo ciclo del tracker. Resultado: las señales quedaron
// OPEN aunque DEBERÍAN haberse cerrado WIN.
//
// Solución: consultar klines 1m de Binance Futures cubriendo todo el período
// desde el created_at de la signal hasta ahora. Para cada vela 1m, revisar
// si high (LONG) o low (SHORT) tocó el TP, y si low (LONG) o high (SHORT)
// tocó el SL. La PRIMERA vela que cumpla decide el cierre. Como bonus, los
// MAE/MFE se calculan REALES (verdad histórica) en lugar de solo los snapshots
// que vio el tracker en-vivo.
//
// Reglas:
// · LONG  → WIN si high de alguna vela >= tp, LOSS si low <= sl
// · SHORT → WIN si low de alguna vela <= tp, LOSS si high >= sl
// · Si la MISMA vela toca TP y SL, prioriza SL (conservador, gap raro)
// · Solo considera velas con openTime >= createdAt de la signal
// · El livePrice se incluye en MAE/MFE para cubrir el gap entre la última
//   vela 1m cerrada y el momento actual (la vela in-progress no viene en
//   fetchLiveCandles porque .slice(0,-1) la descarta).
interface SignalOutcomeFromKlines {
  status:   "OPEN" | "WIN" | "LOSS";  // EXPIRED se decide aparte por tiempo
  hitPrice: number | null;
  closedAt: number | null;             // openTime de la vela que cerró
  mae:      number;                    // MAE real desde klines + livePrice
  mfe:      number;                    // MFE real desde klines + livePrice
}

function checkSignalOutcomeAgainstKlines(
  params: {
    entryPrice:  number;
    tpPrice:     number;
    slPrice:     number;
  ...
  klines1m:  Candle[],   // velas 1m en orden ASC (más vieja primero)
  livePrice: number,     // precio actual para cubrir gap última-vela → ahora
): SignalOutcomeFromKlines {
  const isLong = params.direction === "LONG";

  // Filtrar velas que comienzan en o después del momento de creación de la señal.
  // Si la signal se creó a las 01:02:58, las velas con openTime >= 01:02:00
  // que incluyen ese momento. Como las velas son ASC, basta con filtrar por t.
  const relevantKlines = klines1m.filter(k => k.t >= params.createdAtMs - 60_000);

  // Inicializar MAE/MFE con el valor previo guardado en BD si existe,
  // sino con el livePrice (defecto, igual al modo legacy).
  let mae = params.initialMae ?? livePrice;
  let mfe = params.initialMfe ?? livePrice;

  // Recorrer velas EN ORDEN cronológico para detectar el PRIMER cierre
  for (const k of relevantKlines) {
    // Actualizar MAE/MFE con high/low de la vela
    if (isLong) {
      mae = Math.min(mae, k.l);
      mfe = Math.max(mfe, k.h);
    } else {
      mae = Math.max(mae, k.h);
      mfe = Math.min(mfe, k.l);
    }

    // Chequear si esta vela tocó SL (prioridad sobre TP en gaps)
    const slHit = isLong ? k.l <= params.slPrice : k.h >= params.slPrice;
    if (slHit) {
      return { status: "LOSS", hitPrice: params.slPrice, closedAt: k.t, mae, mfe };
    }
    // Chequear si esta vela tocó TP
    const tpHit = isLong ? k.h >= params.tpPrice : k.l <= params.tpPrice;
    if (tpHit) {
      return { status: "WIN", hitPrice: params.tpPrice, closedAt: k.t, mae, mfe };
    }
  }

  // Ninguna vela cerró. Incluir livePrice en MAE/MFE final para cubrir el gap
  // entre la última vela 1m cerrada (~1min atrás) y el momento actual.
  if (isLong) {
    mae = Math.min(mae, livePrice);
    mfe = Math.max(mfe, livePrice);
  } else {
    mae = Math.max(mae, livePrice);
    mfe = Math.min(mfe, livePrice);
  }

  return { status: "OPEN", hitPrice: null, closedAt: null, mae, mfe };
}
              ...
              if (validProfiles.length > 0) {
                ...
                const res = await fetch("/api/shadow-trades", {
                  ...
                  body: JSON.stringify({
                    ...
                    // Snapshot capital + riesgo (Opción B, 10/06/26)
                    capital_at_signal:  (typeof window !== "undefined" && parseFloat(localStorage.getItem("tp3_capital") || "")) || 10000,
                    risk_pct_at_signal: (typeof window !== "undefined" && parseFloat(localStorage.getItem("tp3_risk")    || "")) || 1,
                ...
                if (res.ok) {
                  ...
                  // Duplicado bloqueado por UNIQUE INDEX server-side. La señal
                  // ya existe en BD (otra instancia/dispositivo la insertó
                  // primero). Considerar éxito: NO liberar la key local para
                  // que el motor no reintente este bucket. Sin este branch,
                  // el finally liberaría la key y entraríamos en loop de POSTs
                  // rechazados.
                  success = true;
                  // eslint-disable-next-line no-console
                  console.log(`[Shadow] Duplicate blocked by server (key=${key})`);
                } else {
                  // eslint-disable-next-line no-console
...
// ── PIPELINE de signals_emitted (regla #24, captura silenciosa) ───────────────
// Captura toda señal que el motor emite como veredicto "ENTRAR" (LONG o SHORT).
// Complementario a Shadow Trading (que captura señales rechazadas). Juntos dan
// visibilidad completa del comportamiento del motor para validación empírica.
//
// Se ejecuta al final de cada load() del motor (cada 5 min y en visibilitychange),
// igual que el pipeline de Shadow. Silencioso por diseño: errores se logean a
// consola pero NUNCA propagan al flujo principal del terminal.
//
// Dedup: 1 captura por dirección por hora calendario UTC (signalEmittedDedupKey).
// Más estricto que "una por vela HTF" pero seguro y suficiente para los volúmenes
// esperados (~5-15 señales/día en pico).
//
// Race condition: se reserva la dedup key ANTES del await fetch + try/finally
// para liberar si el POST falla. Mismo patrón aplicado en Shadow el 08/06/26.
interface SignalEmittedPipelineDeps {
  verdict:      LiveVerdict;
  m15Sig:       SignalDirection;
  ltfSig:       SignalDirection;
  d1Bias:       SignalDirection;
  ...
  livePrice:    number;
  seenKeysRef:  React.MutableRefObject<Set<string>>;
  onExpiring?:  (list: ExpiringSignal[]) => void;
}

async function runSignalEmittedPipeline(deps: SignalEmittedPipelineDeps): Promise<void> {
  const { verdict, m15Sig, ltfSig, d1Bias, h4Bias, atr, htfTf, sessionTag, hasNews, livePrice, seenKeysRef, onExpiring } = deps;

  try {
    // ── 1. DETECTOR ──────────────────────────────────────────────────────
    // Captura señales nuevas cuando el motor emite ENTRAR. Wrapped en IIFE
    // para que los `return` tempranos solo salgan del detector — el tracker
    // de abajo corre siempre, haya o no captura nueva.
    await (async () => {
    // ── GATE TEMPORALIDAD (fix #2, 14/06/26) ─────────────────────────────
    // Solo registramos señales de la temporalidad de operacion (1H). El toggle
    // 4H es solo-ver: cuando htfTf==="4h" NO capturamos (evitaba registrar
    // señales 4H al precio del click, que sesgaban la Performance). El tracker
    // de abajo (seccion 2) corre igual para no dejar señales abiertas sin cerrar.
    if (htfTf !== "1h") return;
    // Solo capturamos cuando el motor dice ENTRAR (veredictoFinal === "ENTRAR")
    // y los datos críticos están completos. Si falta algo, no capturamos.
    if (verdict.veredictoFinal !== "ENTRAR") return;
    if (!verdict.direccion) return;
    if (!verdict.fuerza)    return;
    if (!verdict.levels)    return;

    const dir = verdict.direccion;
    const lev = verdict.levels;

    // Coherencia mínima de los niveles (defensa contra valores raros)
    if (!Number.isFinite(lev.entryPrice) || lev.entryPrice <= 0) return;
    if (!Number.isFinite(lev.slPrice)    || lev.slPrice    <= 0) return;
    if (!Number.isFinite(lev.tpPrice)    || lev.tpPrice    <= 0) return;

    // R:R planeado = distancia al TP / distancia al SL
    const distSL = Math.abs(lev.entryPrice - lev.slPrice);
    const distTP = Math.abs(lev.tpPrice    - lev.entryPrice);
    if (distSL <= 0 || distTP <= 0) return;
    const rrPlanned = distTP / distSL;

    // Dedup por bucket horario UTC
    const key = signalEmittedDedupKey(dir, Date.now());
    if (seenKeysRef.current.has(key)) return;

    // RESERVA INMEDIATA antes del await (fix race condition aprendido 08/06/26)
    seenKeysRef.current.add(key);
    let success = false;

    try {
      // Payload espejado del schema de la tabla signals_emitted
      const body = {
        direction:      dir,
        htf_tf:         htfTf,
        entry_price:    lev.entryPrice,
        sl_price:       lev.slPrice,
        tp_price:       lev.tpPrice,
        sl_pct:         lev.slPct,
        rr_planned:     Math.round(rrPlanned * 100) / 100,
        score_puro:     Math.round(verdict.score),
        score_ajustado: Math.round(verdict.scoreAjustado),
        fuerza:         verdict.fuerza,
        htf_sig:        verdict.htf,
        mtf_sig:        verdict.mtf,
        m15_sig:        m15Sig,
        ltf_sig:        ltfSig,
        d1_bias:        d1Bias,
        h4_bias:        h4Bias,
        rsi_at_entry:   verdict.rsi,
        atr_at_entry:   atr,
        ema200_at:      verdict.ema200,
        liquidez:       verdict.liquidez,
        session_tag:    sessionTag,
        fvg_active:     verdict.fvgActive,
      ...
      if (res.ok) {
        ...
        // eslint-disable-next-line no-console
        console.log(
          `[Signals] Captured ${dir} @ ${lev.entryPrice.toFixed(2)} ` +
          `score=${Math.round(verdict.score)} fuerza=${verdict.fuerza} (key=${key})`
        );
      } else if (res.status === 409) {
        // Duplicado bloqueado por UNIQUE INDEX server-side. La señal ya
        // existe en BD. Considerar éxito: NO liberar la key local para
        // evitar loop de POSTs rechazados.
        success = true;
        // eslint-disable-next-line no-console
        console.log(`[Signals] Duplicate blocked by server (key=${key})`);
      } else {
        // eslint-disable-next-line no-console
        console.warn(`[Signals] POST falló: ${res.status}`);
      }
    } finally {
      // Si NO se confirmó el insert, liberar la reserva para que el próximo
      // ciclo pueda reintentar. La rama exitosa deja la key en el Set.
      if (!success) seenKeysRef.current.delete(key);
    }
    })();

    // ── 2. TRACKER ────────────────────────────────────────────────────────
    // Por cada signal OPEN del usuario: actualiza MAE/MFE si hay nuevo
    // extremo, y cierra con WIN/LOSS/EXPIRED si tocó TP/SL o expiró 24h.
    // Optimización: NO hace PATCH si no cambió nada (evita writes inútiles).
    // Optimización: si la signal cierra, mete el último MAE/MFE en el mismo
    // PATCH del cierre — un solo write por signal por ciclo.
    if (livePrice <= 0) return;
    const trackHeaders = await authHeaders();
    const openRes = await fetch("/api/signals-emitted?status=OPEN", { headers: trackHeaders });
    if (!openRes.ok) return;
    const openSignals = await openRes.json() as Array<{
      id:           string;
      direction:    "LONG" | "SHORT";
      entry_price:  number;
      sl_price:     number;
    ...
    if (!Array.isArray(openSignals) || openSignals.length === 0) { onExpiring?.([]); return; }

    // ── PATCH RETROACTIVO (10/06/26): klines 1m para detectar TP/SL pasados ──
    // El tracker original solo comparaba livePrice contra TP/SL en cada
    // ciclo (5min). Si el precio tocaba un nivel y rebotaba antes del
    // próximo ciclo, el tracker NO lo detectaba (bug observado el 09/06/26
    // con señales SHORT TP=$4191.71 que el precio cruzó momentáneamente).
    //
    // Solución: fetchear klines 1m de Binance Futures cubriendo desde la
    // signal OPEN más vieja hasta ahora. Por cada signal, recorrer las
    // velas en orden cronológico. La primera vela cuyo high/low tocó TP/SL
    // cierra la signal con result_at = openTime de esa vela. Bonus:
    // MAE/MFE se calculan reales (verdad histórica) en lugar de snapshots.
    //
    // Degradación graceful: si Binance falla, seguimos con livePrice (modo
    // legacy). Mejor menos preciso que romper el tracker entero.
    let klines1m: Candle[] | null = null;
    try {
      const nowMs    = Date.now();
      const oldestMs = Math.min(...openSignals.map(s => new Date(s.created_at).getTime()));
      const minutesSpan = Math.ceil((nowMs - oldestMs) / 60_000);
      // Binance Futures cap: 1500 velas. Pedimos minutesSpan + 10 margen,
      // mínimo 60 para que aunque todas las signals sean recientes haya
      // contexto suficiente.
      const limitNeeded = Math.min(1500, Math.max(60, minutesSpan + 10));
      klines1m = await fetchLiveCandles("1m", limitNeeded);
    } catch (e) {
      // eslint-disable-next-line no-console
      console.warn("[Signals] fetch klines 1m falló, fallback livePrice:", e);
      klines1m = null;
    }

    // Acumulador de señales OPEN por vencer (≤2h para las 24h) → banner.
    const nearExpiry: ExpiringSignal[] = [];
    ...
    for (const sig of openSignals) {
      ...
      // ── Calcular outcome + MAE/MFE ──
      // Modo nuevo (klines 1m, verdad histórica) si tenemos klines.
      // Modo legacy (livePrice snapshot) como fallback si Binance falló.
      let newMae:  number;
      let newMfe:  number;
      let outcome: SignalEmittedOutcome;

      if (klines1m && klines1m.length > 0) {
        const kOutcome = checkSignalOutcomeAgainstKlines({
          entryPrice:  sig.entry_price,
          tpPrice:     sig.tp_price,
          slPrice:     sig.sl_price,
          direction:   sig.direction,
          createdAtMs: createdMs,
          initialMae:  sig.mae_price,
          initialMfe:  sig.mfe_price,
        }, klines1m, livePrice);

        newMae = kOutcome.mae;
        newMfe = kOutcome.mfe;

        if (kOutcome.status === "OPEN") {
          // Klines no detectaron cierre. Chequear expiración por tiempo.
          const expiresAt = createdMs + SIGNAL_EMITTED_EXPIRY_MS;
          if (Date.now() >= expiresAt) {
            outcome = { status: "EXPIRED", hitPrice: livePrice, closedAt: Date.now() };
          } else {
            outcome = { status: "OPEN", hitPrice: null, closedAt: null };
          }
        } else {
          // WIN o LOSS detectados con timestamp de la vela exacta
          outcome = {
            status:   kOutcome.status,
            hitPrice: kOutcome.hitPrice,
            closedAt: kOutcome.closedAt,
          };
        }
      } else {
        // FALLBACK: modo original (livePrice snapshot)
        const currMae = sig.mae_price ?? livePrice;
        const currMfe = sig.mfe_price ?? livePrice;
        newMae = isLong ? Math.min(currMae, livePrice) : Math.max(currMae, livePrice);
        newMfe = isLong ? Math.max(currMfe, livePrice) : Math.min(currMfe, livePrice);
        outcome = checkSignalEmittedOutcome({
          entryPrice: sig.entry_price,
          tpPrice:    sig.tp_price,
          slPrice:    sig.sl_price,
          direction:  sig.direction,
      ...
      // Aviso por vencer: si sigue OPEN y le quedan ≤2h, sumarla al banner.
      if (outcome.status === "OPEN") {
        const msLeft = (createdMs + SIGNAL_EMITTED_EXPIRY_MS) - Date.now();
        if (msLeft > 0 && msLeft <= SIGNAL_EXPIRY_WARN_MS) {
          nearExpiry.push({
            id:         sig.id,
            direction:  sig.direction,
            entryPrice: sig.entry_price,
      ...
      // ¿Cambió alguno respecto a lo guardado?
      const maeChanged = sig.mae_price === null || newMae !== sig.mae_price;
      const mfeChanged = sig.mfe_price === null || newMfe !== sig.mfe_price;

      if (outcome.status === "OPEN") {
        // Aún abierta. Solo PATCH si MAE o MFE cambió. Sino, skip.
        if (!maeChanged && !mfeChanged) continue;

        const maePct = ((newMae - sig.entry_price) / sig.entry_price) * 100;
        const mfePct = ((newMfe - sig.entry_price) / sig.entry_price) * 100;
        const patchBody: Record<string, unknown> = {
          id:        sig.id,
          mae_price: Math.round(newMae * 100) / 100,
          mae_pct:   Math.round(maePct * 100) / 100,
          mfe_price: Math.round(newMfe * 100) / 100,
          mfe_pct:   Math.round(mfePct * 100) / 100,
        };

        const r = await fetch("/api/signals-emitted", {
          method:  "PATCH",
          headers: { "Content-Type": "application/json", ...trackHeaders },
          body:    JSON.stringify(patchBody),
        });
        if (!r.ok) {
          // eslint-disable-next-line no-console
          console.warn(`[Signals] PATCH MAE/MFE falló (${sig.id.slice(0,8)}): ${r.status}`);
        }
        continue;
      }

      // ── Cierre (WIN/LOSS/EXPIRED) ──
      if (outcome.hitPrice == null || outcome.closedAt == null) continue;

      const resultPrice = outcome.hitPrice;

      // pnl_pct con signo (positivo = a favor del trade teórico)
      const moveAbs = resultPrice - sig.entry_price;
      const pnlPctRaw = isLong
        ? ( moveAbs / sig.entry_price) * 100
        : (-moveAbs / sig.entry_price) * 100;
      const pnlPct = Math.round(pnlPctRaw * 100) / 100;

      // R-multiple: cuánto ganaste/perdiste en unidades de riesgo inicial.
      // LONG:  (result - entry) / (entry - sl)   → +R en WIN, -1 en LOSS
      // SHORT: (entry - result) / (sl - entry)   → +R en WIN, -1 en LOSS
      const distSL = Math.abs(sig.entry_price - sig.sl_price);
      const rMultiple = distSL > 0
        ? Math.round((isLong ? moveAbs : -moveAbs) / distSL * 100) / 100
        : null;

      const maePctFinal = ((newMae - sig.entry_price) / sig.entry_price) * 100;
      const mfePctFinal = ((newMfe - sig.entry_price) / sig.entry_price) * 100;
      ...
      const closeBody = {
        ...
        // Último MAE/MFE en el mismo PATCH para no hacer 2 writes
        mae_price:    Math.round(newMae * 100) / 100,
        mae_pct:      Math.round(maePctFinal * 100) / 100,
        mfe_price:    Math.round(newMfe * 100) / 100,
        mfe_pct:      Math.round(mfePctFinal * 100) / 100,
      };

      const patchRes = await fetch("/api/signals-emitted", {
        method:  "PATCH",
        headers: { "Content-Type": "application/json", ...trackHeaders },
        body:    JSON.stringify(closeBody),
      });
      if (patchRes.ok) {
        const rStr = rMultiple !== null ? `R=${rMultiple >= 0 ? "+" : ""}${rMultiple.toFixed(2)}` : "R=?";
        // eslint-disable-next-line no-console
        console.log(
          `[Signals] Cerrado ${sig.id.slice(0, 8)} → ${outcome.status} ` +
          `@ ${resultPrice.toFixed(2)} (${pnlPct >= 0 ? "+" : ""}${pnlPct.toFixed(2)}%, ${rStr})`
        );
      } else {
        // eslint-disable-next-line no-console
        console.warn(`[Signals] PATCH cierre falló (${sig.id.slice(0,8)}): ${patchRes.status}`);
      }
    }

    // Publicar al componente las señales OPEN por vencer (puede ser lista vacía).
    onExpiring?.(nearExpiry);
  } catch (e) {
    // Silencioso por diseño — la captura no debe romper el terminal.
    // eslint-disable-next-line no-console
    console.warn("[Signals] pipeline error:", e);
  }
}

// ── Banner: señales OPEN por vencer por TIEMPO (≤2h para las 24h) ───────────
// Solo visual (la versión push queda para #8). Aparece arriba de la columna
// Veredicto y solo cuando hay ≥1 señal por vencer; si la lista llega vacía,
// no renderiza nada. La hora de apertura (COL) ayuda a ubicar la operación en
// MT5; el tiempo restante se recalcula en cada render desde createdAt.
function ExpiryWarning({signals}:{signals:ExpiringSignal[]}){
  if(!signals||signals.length===0) return null;

  const fmtLeft=(createdAt:number):string=>{
    const ms=(createdAt+SIGNAL_EMITTED_EXPIRY_MS)-Date.now();
    if(ms<=0) return "vencida";
    const h=Math.floor(ms/3_600_000);
    const m=Math.floor((ms%3_600_000)/60_000);
    return `vence en ${h}h ${m}m`;
  };
  const fmtOpen=(createdAt:number):string=>{
    try{
      const d=new Date(createdAt);
      const day=d.toLocaleDateString("es-CO",{timeZone:"America/Bogota",day:"2-digit",month:"short"});
      const time=d.toLocaleTimeString("es-CO",{timeZone:"America/Bogota",hour:"2-digit",minute:"2-digit",hour12:false});
      return `abrió ${day} · ${time}`;
    }catch{ return ""; }
  };

  return (
    <div style={{background:T.warnBg,border:`1px solid ${T.warnBorder}`,borderRadius:8,padding:"10px 12px",marginBottom:8}}>
      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
        <span style={{fontSize:13}}>⚠️</span>
        <span style={{fontFamily:SANS,fontSize:11,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",color:T.wait}}>
          {signals.length===1?"1 señal por vencer":`${signals.length} señales por vencer`}
        </span>
      </div>
      {signals.map(s=>(
        <div key={s.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,padding:"6px 0",borderTop:`1px solid ${T.border}`}}>
          <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}>
            <span style={{fontFamily:SANS,fontSize:9,fontWeight:700,color:s.direction==="LONG"?T.up:T.down,background:s.direction==="LONG"?T.upBg:T.dnBg,border:`1px solid ${s.direction==="LONG"?T.upBorder:T.dnBorder}`,padding:"2px 7px",borderRadius:4,whiteSpace:"nowrap"}}>{s.direction}</span>
            <div style={{display:"flex",flexDirection:"column",gap:2,minWidth:0}}>
              <span style={{fontFamily:MONO,fontSize:11,color:T.text}}>@ {s.entryPrice.toFixed(2)}</span>
              <span style={{fontFamily:MONO,fontSize:9,color:T.muted}}>{fmtOpen(s.createdAt)}</span>
            </div>
          </div>
          <span style={{fontFamily:MONO,fontSize:11,fontWeight:700,color:T.wait,whiteSpace:"nowrap"}}>{fmtLeft(s.createdAt)}</span>
        </div>
      ))}
      <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${T.border}`,fontFamily:SANS,fontSize:10,color:T.muted}}>
        Revisá en MT5 — cerrar, breakeven o dejar correr.
      </div>
    </div>
  );
}
...
export default function LiveTerminal({userId,price,connected,notifPerm,notifEnabled}:{userId:string;price:number;connected:boolean;notifPerm:"default"|"granted"|"denied";notifEnabled:boolean}){
  ...
  // Señales OPEN por vencer por tiempo (banner de aviso). Lo llena el tracker.
  const[expiringSignals,setExpiringSignals] = useState<ExpiringSignal[]>([]);
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(saved==="veredicto"||saved==="analisis"){
        setMobileTab(saved);
      }
    }catch{}
  ...
  // Refs persistentes entre remontajes (fix 10/06/26). Antes vivían dentro
  // del componente con useRef(...) — al cambiar de pestaña se reiniciaban
  // y el push notification se re-disparaba. Ahora apuntan al singleton módulo.
  const lastVerdictRef  = __lastVerdictRef;
  const cooldownLongRef = __cooldownLongRef;
  const cooldownShortRef= __cooldownShortRef;
  ...
  // Persiste entre remontajes (singleton módulo, fix 10/06/26).
  const shadowSeenKeysRef = useRef(__shadowSeenKeys);

  // ── Signals Emitted: set de keys ya capturadas (regla #24) ──────────────
  // Mismo patrón que shadowSeenKeysRef. Bucket horario UTC por dirección.
  // Hidratado al montar desde /api/signals-emitted.
  // Persiste entre remontajes (singleton módulo, fix 10/06/26).
  const signalEmittedSeenKeysRef = useRef(__signalEmittedSeenKeys);
  ...
  useEffect(()=>{
    async function load(){
      ...
      try{
        ...
        // ── Signals Emitted: pipeline silencioso (regla #24) ────────────────
        // Captura señales que el motor EMITE como ENTRAR. Complementario a
        // Shadow Trading (que captura RECHAZADAS). Aplica fix race condition.
        void runSignalEmittedPipeline({
          verdict:     v,
          m15Sig,
          ltfSig,
          d1Bias:      d1SigLocal,
          ...
          hasNews,
          livePrice:   priceRef.current,
          seenKeysRef: signalEmittedSeenKeysRef,
          onExpiring:  setExpiringSignals,
        });
      }catch(err){
  ...
  useEffect(()=>{
    ...
    (async () => {
      try {
        ...
        // MERGE en vez de reasignación (fix 10/06/26). Si el motor ya agregó
        // keys en runtime ANTES de que terminara esta hidratación inicial,
        // reasignar (`= set`) las perdería. Hacer .add() preserva todas.
        // Además mantiene la referencia al singleton módulo.
        for (const k of set) shadowSeenKeysRef.current.add(k);
      } catch (e) {
  ...
  // Signals Emitted: hidratación inicial del set de keys (regla #24)
  // Mismo patrón que Shadow. Trae las últimas señales emitidas y reconstruye
  // las keys con su created_at original. Evita re-insertar señales ya capturadas
  // si el usuario refresca el navegador dentro de la misma hora UTC.
  useEffect(()=>{
    if(!userId)return;
    let cancelled = false;
    (async () => {
      try {
        const headers = await authHeaders();
        const res = await fetch("/api/signals-emitted?limit=100", { headers });
        if (!res.ok) return;
        const data = await res.json() as Array<{
          direction:  "LONG" | "SHORT";
          created_at: string;
        ...
        for (const s of data) {
          ...
          if (!Number.isFinite(ms)) continue;
          set.add(signalEmittedDedupKey(s.direction, ms));
        }
        // MERGE en vez de reasignación (fix 10/06/26). Mismo razonamiento
        // que en la hidratación de Shadow: preserva keys que el motor pudo
        // agregar antes de que terminara este fetch.
        for (const k of set) signalEmittedSeenKeysRef.current.add(k);
      } catch (e) {
        // Silencioso — si la hidratación falla, peor caso: se inserta una
        // captura duplicada (pero no rompe nada). Mejor que bloquear el UI.
        // eslint-disable-next-line no-console
        console.warn("[Signals] hidratación inicial falló:", e);
      }
    })();
    return () => { cancelled = true; };
  },[userId]);
  ...
  return(
    <>
      <style>{`
        ...
        .tp3-root{display:grid;grid-template-columns:1fr 1.3fr;grid-template-rows:1fr;height:100%;width:100%;max-width:1200px;margin:0 auto;overflow:hidden;padding:0 24px;box-sizing:border-box;border-left:1px solid var(--tp3-border);border-right:1px solid var(--tp3-border);}
        .tp3-main{grid-column:1;grid-row:1;}
        .tp3-sidebar{grid-column:2;grid-row:1;}
        @media(max-width:700px){
          /* Mobile: 1 col, mostrar Veredicto O Análisis según data-mobile-tab */
          .tp3-root{grid-template-columns:1fr;padding:0;max-width:none;margin:0;border-left:none;border-right:none;}
          .tp3-sidebar,.tp3-main{display:none!important;}
        ...
        /* LANDSCAPE iPhone: cuando el celular está horizontal (alto ≤500px),
           mostrar las 2 columnas lado a lado en lugar del modo tab-único. */
        @media(orientation:landscape) and (max-height:500px){
          .tp3-root{grid-template-columns:1fr 1fr!important;max-width:none;border-left:none;border-right:none;}
          .tp3-main{display:flex!important;grid-column:1!important;grid-row:1!important;}
          .tp3-sidebar{display:flex!important;grid-column:2!important;grid-row:1!important;}
        }
      `}</style>
      ...
      <div className="tp3-root" data-mobile-tab={mobileTab} style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-sidebar"
          style={{background:T.bg,borderLeft:`1px solid ${T.border}`,overflowY:"auto",padding:"8px 8px",display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          <Card style={{marginBottom:4}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              {[
                ...
                <div key={label} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"3px 7px",borderRadius:5,background:T.s2}}>
                  <div>
          ...
          <Card style={{marginBottom:4}}>
            ...
            <div style={{display:"flex",gap:3,marginBottom:6}}>
              {(["4h","1h"]as const).map(tf=>(
                <button key={tf} onClick={()=>handleHtfChange(tf)} style={{
                  flex:1,padding:"2px 0",borderRadius:5,cursor:"pointer",fontFamily:MONO,fontSize:9,fontWeight:700,
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              ...
              <div>
                <span style={{fontFamily:MONO,fontSize:11,color:T.muted,fontWeight:700,minWidth:32}}>D1</span>
                <span style={{fontFamily:SANS,fontSize:9,color:T.dim,marginLeft:6}}>Sesgo diario</span>
              </div>
            ...
            {mtfRows.map(({key,tf,rol,sig})=>(
              <div key={key} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                ...
                <div>
                  <span style={{fontFamily:MONO,fontSize:11,color:T.muted,fontWeight:700,minWidth:32}}>{tf}</span>
                  <span style={{fontFamily:SANS,fontSize:9,color:T.dim,marginLeft:6}}>{rol}</span>
                  {key==="h4"&&h4Penalty&&(
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"3px 0",borderBottom:`1px solid ${T.border}`}}>
              <div>
                <span style={{fontFamily:MONO,fontSize:11,color:T.muted,fontWeight:700,minWidth:32}}>EST</span>
                <span style={{fontFamily:SANS,fontSize:9,color:T.dim,marginLeft:6}}>Estructura</span>
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"3px 0",borderBottom:`1px solid ${T.border}`}}>
              <div>
                <span style={{fontFamily:MONO,fontSize:11,color:T.muted,fontWeight:700,minWidth:32}}>FVG</span>
                <span style={{fontFamily:SANS,fontSize:9,color:T.dim,marginLeft:6}}>Desequilibrio</span>
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"3px 0",borderBottom:`1px solid ${T.border}`}}>
              <div>
                <span style={{fontFamily:MONO,fontSize:11,color:T.muted,fontWeight:700,minWidth:32}}>PDH</span>
                <span style={{fontFamily:SANS,fontSize:9,color:T.dim,marginLeft:6}}>Liquidez</span>
              </div>
          ...
          {/* Checklist al final del sidebar (tab Análisis en mobile, columna derecha en desktop). */}
          <Checklist
        ...
        <div className="tp3-main"
          ...
          <ExpiryWarning signals={expiringSignals}/>
