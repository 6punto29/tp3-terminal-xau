...
//
// HIDE-ON-SCROLL (10/06/26): el nav se esconde al scrollear hacia abajo y
// reaparece al subir. Patrón X/Twitter/Instagram. Parámetros:
//   · THRESHOLD 50px: scroll acumulado abajo antes de esconder (evita
//     parpadeos por micro-scrolls).
//   · TOP_GUARD 30px: cuando scrollY < 30, nav siempre visible (override).
//   · Velocidad: 250ms ease-out (sensación natural).
// Implementación: useRef + manipulación directa del DOM via style.transform.
// CERO re-renders del componente. Listener passive en window.scroll.
// Al cambiar de tab (prop `active`), nav se fuerza visible (defensa).
...
function MobileBottomNav({active,onChange}:{active:MobileTab;onChange:(t:MobileTab)=>void}){
  ...
  // Scroll listener — manipula el DOM directamente, no re-renderiza.
  //
  // IMPORTANTE: el scroll NO ocurre en window. El terminal tiene un layout
  // grid con .tp3-root{overflow:hidden} y 3 containers internos
  // (.tp3-sidebar, .tp3-main, .tp3-right) que tienen height:100% + overflowY:auto
  // cada uno. En mobile (<700px) solo uno de los 3 está visible por vez
  // (CSS hace display:none/flex según data-mobile-tab del root).
  //
  // Por qué `document` con capture en vez de querySelectorAll al montar:
  //   · Los eventos `scroll` NO burbujean (no suben al padre), por eso no
  //     se pueden escuchar en bubbling phase desde arriba. PERO sí se pueden
  //     capturar en CAPTURE phase desde document — el evento baja por captura
  //     hasta el elemento que scrolleó, y lo interceptamos en el camino.
  //   · Esto atrapa el scroll de CUALQUIER container interno sin importar
  //     cuál esté montado/visible en el momento, y sin re-enganchar listeners
  //     cuando el usuario cambia de tab.
  //   · Robusto en PWA iOS standalone (el querySelectorAll al montar fallaba
  //     si el container estaba display:none o aún no existía).
  useEffect(() => {
    const THRESHOLD = 50;
    const TOP_GUARD = 30;
    ...
    function handleScroll(e: Event) {
      ...
      // El target del scroll es el container interno que scrolleó.
      // Ignoramos scrolls que no vengan de un elemento con scrollTop útil.
      const target = e.target as HTMLElement;
      if (!target || typeof target.scrollTop !== "number") return;

      const currentScroll = target.scrollTop;

      // Top guard: cerca del top, siempre visible
      if (currentScroll < TOP_GUARD) {
      ...
      const direction = currentScroll > lastScrollTopRef.current ? "down" : "up";
    ...
    // capture:true → interceptamos el scroll en fase de captura desde document.
    // passive:true → no bloqueamos el scroll de iOS.
    document.addEventListener("scroll", handleScroll, { capture: true, passive: true });
    return () => document.removeEventListener("scroll", handleScroll, { capture: true });
  }, []);

  // Defensa: al cambiar de tab, forzar nav visible (escenario raro pero
  // posible si el usuario llega a otra tab con scroll aún bajo).
  useEffect(() => {
    const nav = navRef.current;
    if (nav && isHiddenRef.current) {
      nav.style.transform = "translateY(0)";
      isHiddenRef.current = false;
      scrollDeltaRef.current = 0;
    }
  ...
  return(
    <nav ref={navRef} className="tp3-mobile-bottom-nav" style={{
