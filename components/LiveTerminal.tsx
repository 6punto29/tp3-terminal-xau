...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  const[ltfTf, setLtfTf] = useState<"1h"|"15m"|"5m">("15m");
  const[slPct, setSlPct] = useState(1);
  const[tpPct, setTpPct] = useState(4);

  // LTF se ajusta automáticamente cuando cambia HTF
  const handleHtfChange = (tf: "4h"|"1h") => {
    setHtfTf(tf);
    if(tf === "4h") setLtfTf("1h");
    else setLtfTf("15m");
  };

  const ltfOptions: {value:"1h"|"15m"|"5m"; label:string}[] = htfTf === "4h"
    ? [{value:"1h",label:"1H"}]
    ...
    async function load(){
      ...
      try{
        ...
        const[htfC,mtfC,m15C,ltfC]=await Promise.all([
          fetchLiveCandles(htfTf,250),fetchLiveCandles(mtfTf,250),
          fetchLiveCandles("15m",150),fetchLiveCandles(ltfTf,120)]);
  ...
  const mtfRows=[
    {tf:htfTf.toUpperCase(), rol:"Tendencia",    sig:(signal?.htf??"WAIT") as MTFSig},
    {tf:htfTf==="4h"?"1H":"15M", rol:"Confirmación", sig:(signal?.mtf??"WAIT") as MTFSig},
    {tf:"15M", rol:"Refinamiento", sig:(signal?.m15??"WAIT") as MTFSig},
    {tf:ltfTf.toUpperCase(), rol:"Entrada", sig:(signal?.ltf??"WAIT") as MTFSig},
  ];

  return(
    <>
      <style>{`
        .tp3-root{display:grid;grid-template-columns:210px 1fr 300px;grid-template-rows:1fr;height:100%;overflow:hidden;}
      ...
      <div className="tp3-root" style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-sidebar"
          ...
          <Card style={{marginBottom:6}}>
            <SecTitle>Configuracion</SecTitle>
            {/* HTF */}
            <div style={{marginBottom:6}}>
              <div style={{fontFamily:SANS,fontSize:8,color:T.muted,marginBottom:3,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>HTF Tendencia</div>
              <div style={{display:"flex",gap:3}}>
                {(["4h","1h"] as const).map(tf=>(
                  <button key={tf} onClick={()=>handleHtfChange(tf)} style={{
                    flex:1,padding:"4px 0",borderRadius:5,border:`1px solid ${htfTf===tf?T.gold:T.border}`,
            ...
            {/* LTF */}
            <div style={{marginBottom:6}}>
              <div style={{fontFamily:SANS,fontSize:8,color:T.muted,marginBottom:3,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>LTF Entrada</div>
              <div style={{display:"flex",gap:3}}>
                {ltfOptions.map(({value,label})=>(
                  <button key={value} onClick={()=>setLtfTf(value)} style={{
                    flex:1,padding:"4px 0",borderRadius:5,border:`1px solid ${ltfTf===value?T.accent:T.border}`,
            ...
            {/* SL */}
            <div style={{marginBottom:6}}>
              <div style={{fontFamily:SANS,fontSize:8,color:T.muted,marginBottom:3,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>SL %</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:2}}>
                {SL_OPTIONS.map(v=>(
                  <button key={v} onClick={()=>setSlPct(v)} style={{
                    padding:"3px 5px",borderRadius:4,border:`1px solid ${slPct===v?T.down:T.border}`,
            ...
            {/* TP */}
            <div>
              <div style={{fontFamily:SANS,fontSize:8,color:T.muted,marginBottom:3,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>TP %</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:2}}>
                {TP_OPTIONS.map(v=>(
                  <button key={v} onClick={()=>setTpPct(v)} style={{
                    padding:"3px 5px",borderRadius:4,border:`1px solid ${tpPct===v?T.up:T.border}`,
