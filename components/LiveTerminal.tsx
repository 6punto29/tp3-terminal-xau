...
export default function LiveTerminal({ userId }: LiveTerminalProps) {
  ...
  // Stub score — replace with real conviction score from BacktestLaboratory
  const score = 7;

  // Populate session on client only (avoids SSR hydration mismatch)
  useEffect(() => {
  ...
  // Derive a simple live signal from WebSocket price movement
  // In production this would call /api/backtest or a dedicated signals endpoint
  useEffect(() => {
    if (!ws.kline) return;
    const delta = ws.kline.c - ws.kline.o;
    const pct   = ws.kline.o > 0 ? (delta / ws.kline.o) * 100 : 0;
    // Simplified live signal — replace with real MTF logic
    const htf: MTFSig = pct > 0.3 ? "UP" : pct < -0.3 ? "DOWN" : "WAIT";
      ...
      <div style={{
        ...
        <Card>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: 4 }}>
            {[
              { lbl:"WR",  val:"57%",  c:T.up   },
              { lbl:"Ops", val:"42",   c:T.text  },
              { lbl:"P&L", val:"+23%", c:T.up   },
              { lbl:"Wins",val:"24",   c:T.up   },
            ].map(({ lbl, val, c }) => (
