...
function calcSignalScore(htf:MTFSig,mtf:MTFSig,ltf:MTFSig,rsi:number|null,ema200:number|null,price:number,session:Session):number{
  let s=0;
  if(htf!=="WAIT")s+=2;
  if(htf!=="WAIT"&&mtf===htf)s+=2;
  if(htf!=="WAIT"&&ltf===htf)s+=1;
  if(ema200!=null&&price>0){if((htf==="UP"&&price>ema200)||(htf==="DOWN"&&price<ema200))s+=2;}
  if(rsi!=null){if((htf==="UP"&&rsi<70)||(htf==="DOWN"&&rsi>30))s+=2;}
...
// OPERATION FORM
interface OpFormProps{livePrice:number;userId:string;onSaved:(op:Operation)=>void;}
  ...
  return(
    <Card style={{borderLeft:`3px solid ${border}`,marginBottom:6}}>
      ...
      <div style={{display:"flex",gap:8}}>
        {[{label:"HTF 4H",sig:signal.htf},{label:"MTF 1H",sig:signal.mtf},{label:"LTF 5M",sig:signal.ltf}].map(({label,sig})=>(
...
export default function LiveTerminal({userId}:{userId:string}){
  ...
  const signalScore=signal?calcSignalScore(signal.htf,signal.mtf,signal.ltf,signal.rsi,signal.ema200,ws.price,session):0;
  ...
  useEffect(()=>{
    async function load(){
      setSigError(null);
      try{
        const[htfC,mtfC,ltfC]=await Promise.all([
          fetchLiveCandles("4h",250),fetchLiveCandles("1h",250),fetchLiveCandles("5m",120)]);
        if(htfC.length<50||mtfC.length<50){setSigError("Datos insuficientes de Binance");return;}
        const htfInd=precompute(htfC),mtfInd=precompute(mtfC),ltfInd=precompute(ltfC);
        ...
        setSignal({htf:v.htf,mtf:v.mtf,ltf:ltfSig,verdict:v.verdict,
  ...
  const mtfRows=[
    {tf:"4H",sig:(signal?.htf??"WAIT") as MTFSig},
    {tf:"1H",sig:(signal?.mtf??"WAIT") as MTFSig},
    {tf:"5M",sig:(signal?.ltf??"WAIT") as MTFSig},
  ];
    ...
    <>
      <style>{`
        ...
        @media(max-width:700px){
          .tp3-root{grid-template-columns:1fr;grid-template-rows:44px 1fr 52px;}
          .tp3-sidebar,.tp3-main,.tp3-right{grid-column:1!important;grid-row:2!important;display:none!important;}
          .tp3-sidebar.mob-active,.tp3-main.mob-active,.tp3-right.mob-active{display:flex!important;}
          .tp3-mob-tabs{display:flex;grid-column:1;grid-row:3;}
        }
      `}</style>
      ...
      <div className="tp3-root" style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-topbar" style={{display:"flex",alignItems:"center",justifyContent:"space-between",
          ...
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            ...
            <button className="tp3-theme-btn" onClick={toggleTheme} style={{fontFamily:SANS}}>
        ...
        <div className={`tp3-sidebar${mobileTab==="estado"?" mob-active":""}`}
          ...
          <Card style={{marginBottom:6}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
              ...
              <span style={{fontFamily:SANS,fontSize:11,color:T.muted}}>{ws.connected?"Binance Spot WS":"Desconectado"}</span>
            </div>
            <div style={{fontFamily:MONO,fontSize:10,color:T.muted,lineHeight:1.6}}>
              <div>XAUUSDT</div><div>Stream: kline_1m</div>
              {ws.lastUpdate>0&&<div>{new Date(ws.lastUpdate).toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit",second:"2-digit"})}</div>}
            </div>
          ...
          <Card style={{marginBottom:6}}>
            ...
            {mtfRows.map(({tf,sig})=>(
              <div key={tf} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 0",borderBottom:tf!=="5M"?`1px solid ${T.border}`:undefined}}>
        ...
        {/* RIGHT PANEL — overflow hidden, sin scroll */}
        <div className={`tp3-right${mobileTab==="operar"?" mob-active":""}`}
          style={{background:T.bg,borderLeft:`1px solid ${T.border}`,
            overflow:"hidden",padding:"8px 8px",display:"flex",flexDirection:"column"}}>
        ...
        {/* MOBILE TABS */}
        <div className="tp3-mob-tabs" style={{background:T.s1,borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"stretch"}}>
          {(["estado","senal","operar"]as MobileTab[]).map(id=>(
            <button key={id} onClick={()=>setMobileTab(id)} style={{flex:1,border:"none",cursor:"pointer",
              background:mobileTab===id?T.s2:"transparent",color:mobileTab===id?T.text:T.muted,
              fontFamily:SANS,fontSize:11,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",
