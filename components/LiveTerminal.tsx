...
function getNextSession():{label:string;mins:number}{
  ...
  // Fix viernes tarde (v6): después del cierre NY el viernes (>=18:00 UTC = 13:00 COL),
  // el próximo evento es LDN del LUNES, no "mañana" (sábado).
  // Saltamos sábado completo (1440min) + domingo completo (1440min) + 420min hasta LDN.
  if(day===5&&u>=1080){const minsToMon=(DAY-u)+DAY+DAY+420;return{label:"LDN Lunes",mins:minsToMon};}
  // Días de semana
...
function MT5Block({signal,score,price,slPct,tpPct,refreshKey,htfTf,d1Bias,h4Bias}:MT5BlockProps){
  ...
  if(!signal||score<getScoreThreshold(signal.htf,signal.mtf)||signal.verdict==="ESPERAR")return null;
  const levels=signal.levels;
  const isLong=signal.verdict==="ENTRAR LONG";
