...
import { getLiveVerdict, mtfSignalAt, htfSignalAt, calcSignalScore, getScoreThreshold } from "@/lib/engine/signals";
...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  useEffect(()=>{
    ...
    const threshold=getScoreThreshold(signal.htf,signal.mtf);
    const verdict:Verdict=signalScore>=threshold?signal.verdict:"ESPERAR";
