"use client";
// ─────────────────────────────────────────────────────────────────────────────
// components/LiveTerminal.tsx  — v2.1
//
// Cambios vs v2.0:
//  · Carga velas 4H + 1H + 5M directo desde Binance Futures (sin proxy)
//  · Usa precompute() + getLiveVerdict() + mtfSignalAt() — motor real
//  · MTF Matrix muestra señal real por timeframe
//  · Score calculado desde indicadores (no hardcodeado)
//  · RSI real en checklist
// ─────────────────────────────────────────────────────────────────────────────
...
type Strength    = "FUERTE" | "MODERADO" | "DÉBIL";
...
// ─────────────────────────────────────────────────────────────────────────────
// BINANCE FUTURES — carga directa (igual que BacktestLaboratory)
// ─────────────────────────────────────────────────────────────────────────────
...
async function fetchLiveCandles(tf: string, limit = 250): Promise<Candle[]> {
  const r = await fetch(
    `${BINANCE_FUTURES}?symbol=XAUUSDT&interval=${tf}&limit=${limit}`
  );
  if (!r.ok) throw new Error(`Binance ${r.status} (${tf})`);
  const d = (await r.json()) as number[][];
  if (!Array.isArray(d) || !d.length) return [];
  // Excluir la vela actual (incompleta)
  return d.slice(0, -1).map((k) => ({
...
function calcSignalScore(
  htf: MTFSig, mtf: MTFSig, ltf: MTFSig,
  rsi: number | null, ema200: number | null,
  price: number, session: Session
): number {
  let s = 0;

  // HTF no neutral → base del análisis
  if (htf !== "WAIT") s += 2;

  // MTF confirma al HTF → confluencia principal
  if (htf !== "WAIT" && mtf === htf) s += 2;

  // LTF confirma → entrada más precisa
  if (htf !== "WAIT" && ltf === htf) s += 1;

  // EMA200 confirma el sesgo
  if (ema200 != null && price > 0) {
    const ok = (htf === "UP" && price > ema200) || (htf === "DOWN" && price < ema200);
    if (ok) s += 2;
  }

  // RSI sin contradicción (no overbought en LONG, no oversold en SHORT)
  if (rsi != null) {
    const ok = (htf === "UP" && rsi < 70) || (htf === "DOWN" && rsi > 30);
    if (ok) s += 2;
  }
...
function Checklist({ session, signal, price, signalScore, hasNews }: ChecklistProps) {
  const ema200 = signal?.ema200 ?? null;
  const rsi    = signal?.rsi    ?? null;
  ...
  const c4 = rsi != null && rsi >= 30 && rsi <= 70;
  const c5 = !hasNews;
  const c6 = signalScore >= 6;
  ...
  const items  = [
    ...
    "EMA 200 · sesgo claro",
    "HTF + MTF alineados",
    `RSI sin contradicción${rsi != null ? ` (${rsi.toFixed(0)})` : ""}`,
    "Sin noticia 30M",
    `Score ≥ 6 (actual: ${signalScore})`,
  ];
    ...
    <Card>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap: 4, marginBottom: 8 }}>
        {[
          ...
          { l:"Ganancia $", v: gainPts > 0 ? `$${(gainPts*100).toFixed(0)}` : "--", c: T.up   },
          { l:"R:R",        v: rr > 0      ? `${rr.toFixed(2)}:1`           : "--", c: rr >= 2.5 ? T.up : T.wait },
          { l:"Lotes XAU",  v: lotSize > 0 ? `${lotSize.toFixed(2)}L`       : "--", c: T.gold },
          { l:"$ Riesgo",   v: dollarR > 0 ? `$${dollarR.toFixed(0)}`       : "--", c: T.down },
          { l:"R:R ratio",  v: rr > 0      ? `${rr.toFixed(2)}:1`           : "--", c: rr >= 2 ? T.up : T.muted },
        ].map(({ l, v, c }) => (
    ...
    <Card style={{ borderLeft: `3px solid ${border}`, marginBottom: 6 }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom: 8, gap: 8 }}>
        <div>
          ...
          <div style={{ marginTop: 5, display:"flex", alignItems:"center", gap: 6 }}>
            <Badge color={strengthColor[signal.strength]}>{signal.strength}</Badge>
            <span style={{ fontFamily: MONO, fontSize: 10, color: score >= 6 ? T.up : T.wait }}>
              Score {score}/10
            </span>
          </div>
        </div>
        <div style={{ textAlign:"right" }}>
          ...
          {signal.ema200 != null && (
            <div style={{ marginTop: 4, fontFamily: MONO, fontSize: 9, color: T.muted }}>
              EMA200 {signal.ema200.toFixed(0)}
            </div>
          )}
        </div>
      </div>
      <div style={{ display:"flex", gap: 8 }}>
        <div style={{ display:"flex", alignItems:"center", gap: 4 }}>
          <span style={{ fontFamily: SANS, fontSize: 9, color: T.muted }}>HTF 4H</span>
          <MTFSigBadge sig={signal.htf} />
        </div>
        <div style={{ display:"flex", alignItems:"center", gap: 4 }}>
          <span style={{ fontFamily: SANS, fontSize: 9, color: T.muted }}>MTF 1H</span>
          <MTFSigBadge sig={signal.mtf} />
        </div>
        <div style={{ display:"flex", alignItems:"center", gap: 4 }}>
          <span style={{ fontFamily: SANS, fontSize: 9, color: T.muted }}>LTF 5M</span>
          <MTFSigBadge sig={signal.ltf} />
        </div>
      </div>
    </Card>
  );
...
export default function LiveTerminal({ userId }: LiveTerminalProps) {
  ...
  // Score se calcula derivado del signal + session + precio en vivo
  const signalScore = signal
    ? calcSignalScore(
        signal.htf, signal.mtf, signal.ltf,
        signal.rsi, signal.ema200,
        ws.price, session
      )
    : 0;
  ...
  // ── Cargar señales usando motor real ──
  useEffect(() => {
    async function loadLiveSignals() {
      setSigError(null);
      try {
        // Fetch paralelo: 4H (HTF) + 1H (MTF) + 5M (LTF)
        const [htfCandles, mtfCandles, ltfCandles] = await Promise.all([
          fetchLiveCandles("4h", 250),
          fetchLiveCandles("1h", 250),
          fetchLiveCandles("5m", 120),
        ]);

        if (htfCandles.length < 50 || mtfCandles.length < 50) {
          setSigError("Datos insuficientes de Binance");
          return;
        }

        // Precomputar indicadores por timeframe
        const htfInd = precompute(htfCandles);
        const mtfInd = precompute(mtfCandles);
        const ltfInd = precompute(ltfCandles);

        // Señal HTF + MTF usando el motor completo (RSI, EMA, Bollinger, MACD)
        const verdict = getLiveVerdict(
          htfInd, mtfInd,
          htfCandles.length - 1,
          mtfCandles.length - 1
        );

        // Señal LTF (5M) — misma lógica MTF
        const ltfSig = ltfCandles.length >= 50
          ? mtfSignalAt(ltfInd, ltfCandles.length - 1)
          : "WAIT";
    ...
    loadLiveSignals();
    // Refrescar cada 5 minutos (sincronizado con cierre de vela 5M)
    const iv = setInterval(loadLiveSignals, 5 * 60 * 1000);
    return () => clearInterval(iv);
  }, []);
  ...
  // ── MTF Matrix: señales reales por TF ──
  const mtfRows: { tf: string; sig: MTFSig }[] = [
    { tf: "4H",  sig: signal?.htf ?? "WAIT" },
    { tf: "1H",  sig: signal?.mtf ?? "WAIT" },
    { tf: "5M",  sig: signal?.ltf ?? "WAIT" },
  ];
  ...
  const sidebarContent = (
    <>
      <Card style={{ marginBottom: 6 }}>
        ...
        {sigError && (
          <div style={{
            ...
            background: T.dnBg, border: `1px solid ${T.dnBorder}`,
            fontFamily: MONO, fontSize: 9, color: T.down,
          }}>
            ⚠ {sigError}
          </div>
        )}
      ...
      <Card style={{ marginBottom: 6 }}>
        ...
        <div style={{
          marginTop: 6, padding: "4px 8px", borderRadius: 5, textAlign:"center",
          fontFamily: MONO, fontSize: 10, fontWeight: 700,
          background: signalScore >= 6 ? T.upBg : T.s2,
          color:      signalScore >= 6 ? T.up   : T.muted,
    ...
    <>
      ...
      <style>{`
        ...
        @media (max-width: 1100px) {
          .tp3-root { grid-template-columns: 1fr 280px; }
        ...
        @media (max-width: 700px) {
          ...
          .tp3-main.mob-active,
          .tp3-right.mob-active { display: flex !important; }
          .tp3-mob-tabs {
            display: flex;
            grid-column: 1;
            grid-row: 3;
