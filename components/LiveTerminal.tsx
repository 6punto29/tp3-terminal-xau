...
function OperationForm({livePrice,userId,onSaved,fillHeight,slPct=1,tpPct=4,structureSL,structureTP}:OpFormProps){
  ...
  const[capital,setCap]=useState(()=>{ try{ return localStorage.getItem('tp3_capital')||'10000'; }catch{ return '10000'; } });const[riskPct,setRisk]=useState(()=>{ try{ return localStorage.getItem('tp3_risk')||'1'; }catch{ return '1'; } });
  ...
  const fillLive=()=>{if(livePrice>0){setEntry(livePrice.toFixed(2));setSL("");setTP("");}};
    ...
    <Card style={{padding:"7px 10px",...(fillHeight?{flex:1,display:"flex",flexDirection:"column"}:{})}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:5}}>
        <div><label style={lbl}>Capital $</label>
          <input type="number" value={capital} placeholder="10000" onChange={e=>{setCap(e.target.value);try{localStorage.setItem('tp3_capital',e.target.value);}catch{}}} style={inp}/></div>
        <div><label style={lbl}>Riesgo %</label>
          <input type="number" value={riskPct} placeholder="1" onChange={e=>{setRisk(e.target.value);try{localStorage.setItem('tp3_risk',e.target.value);}catch{}}} style={inp}/></div>
      </div>
...
function CopyTerminalBtn(p:CopyTerminalProps){
  ...
  const copy=()=>{
    ...
    const txt=[
      ...
      `📈 MTF MATRIX:`,
      `  D1:  ${d1lbl}`,
      `  ${p.htfTf.toUpperCase()}: ${s?.htf??"WAIT"} · ${mtfTf}: ${s?.mtf??"WAIT"} · 15M: ${s?.m15??"WAIT"} · ${p.ltfTf.toUpperCase()}: ${s?.ltf??"WAIT"}`,
      ...
      ``,
      `📊 CUENTA: ${p.totalOps} ops · WR ${p.wr}% · P&L ${p.pnlTotal>=0?"+":""}${p.pnlTotal.toFixed(1)}%`,
      `━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    ].filter(l=>l!=="").join("\n");
...
interface EditModalProps{
  op:Operation;
  onClose:()=>void;
  onSave:(updated:{id:string;direccion:Direction;precio_entrada:number;sl:number;tp:number;resultado:OpsResult;pnl:number|null})=>void;
}
function EditModal({op,onClose,onSave}:EditModalProps){
  ...
  const handleSave=async()=>{
    const eNum=parseFloat(entry);const slNum=parseFloat(sl);const tpNum=parseFloat(tp);
    if(!eNum||!slNum||!tpNum){return;}
    let pnl:number|null=null;
    if(resultado==="TP"){
      const pts=dir==="LONG"?Math.abs(tpNum-eNum):Math.abs(eNum-tpNum);
      pnl=eNum>0?(pts/eNum)*100:0;
    }else if(resultado==="SL"){
      const pts=dir==="LONG"?Math.abs(eNum-slNum):Math.abs(slNum-eNum);
      pnl=eNum>0?-(pts/eNum)*100:0;
    }else if(resultado==="MANUAL"){
      const cNum=parseFloat(cierre);
    ...
    setSaving(true);
    try{onSave({id:op.id,direccion:dir,precio_entrada:eNum,sl:slNum,tp:tpNum,resultado,pnl});}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:1000,
      ...
      <div style={{background:T.s1,border:`1px solid ${T.border}`,borderRadius:10,
        padding:"18px 20px",width:290,boxShadow:"0 20px 60px rgba(0,0,0,0.6)"}}>
        ...
        <div style={{marginBottom:8}}>
          <label style={lbl}>Precio entrada</label>
          <input type="number" value={entry} onChange={e=>setEntry(e.target.value)} style={inp}/>
        </div>
        ...
        {resultado==="MANUAL"&&(
          <div style={{marginBottom:8}}>
            ...
            <div style={{fontFamily:MONO,fontSize:8,color:T.dim,marginTop:3}}>
...
function HistoryList({ops,onUpdate,onEdit,onDelete}:HistoryListProps){
        ...
        <div key={op.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:6,
          ...
          {op.resultado==null?(
            ...
            <span style={{fontFamily:MONO,fontWeight:700,fontSize:10,color:op.pnl!=null&&op.pnl>=0?T.up:T.down}}>
              {op.resultado} {op.pnl!=null?`${op.pnl>=0?"+":""}${op.pnl.toFixed(2)}%`:""}
            </span>
          )}
          {/* ✏️ Editar */}
          <button onClick={()=>onEdit(op)} title="Editar" style={{
            background:"none",border:`1px solid ${T.border}`,borderRadius:4,
            padding:"2px 5px",cursor:"pointer",fontSize:10,color:T.muted,
            lineHeight:1,flexShrink:0}}>✏️</button>
          {/* 🗑️ Borrar */}
          <button onClick={()=>onDelete(op.id)} title="Eliminar" style={{
            background:"none",border:`1px solid ${T.border}`,borderRadius:4,
            padding:"2px 5px",cursor:"pointer",fontSize:10,color:T.down,
            lineHeight:1,flexShrink:0}}>🗑️</button>
        </div>
      ))}
    </div>
...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  useEffect(()=>{
    async function load(){
      ...
      try{
        ...
        const v=getLiveVerdict(htfInd,mtfInd,htfC.length-1,mtfC.length-1,price,d1SigLocal,sessionLevels,slPct,tpPct);
  ...
  const handleDelete=useCallback(async(id:string)=>{
    ...
    try{
      await fetch(`/api/operations?id=${id}`,{method:"DELETE",headers:{"x-user-id":userId}});
      setOps(p=>p.filter(o=>o.id!==id));
    }catch(e){console.error(e);}
  },[userId]);

  const handleEditSave=useCallback(async(updated:{id:string;direccion:Direction;precio_entrada:number;sl:number;tp:number;resultado:OpsResult;pnl:number|null})=>{
    try{
      await fetch("/api/operations",{method:"PATCH",headers:{"Content-Type":"application/json","x-user-id":userId},
      ...
      <div className="tp3-root" style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-main"
          ...
          <Card>
            ...
            <HistoryList ops={ops} userId={userId} onUpdate={handleUpdate} onEdit={setEditingOp} onDelete={handleDelete}/>
          </Card>
        </div>
