        ...
        <div className="tp3-sidebar"
          ...
          <Card style={{marginBottom:6}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"5px 0",borderBottom:`1px solid ${T.border}`}}>
              <div>
                <span style={{fontFamily:MONO,fontSize:10,color:T.muted,fontWeight:700,minWidth:28}}>FVG</span>
                <span style={{fontFamily:SANS,fontSize:8,color:T.dim,marginLeft:5}}>Desequilibrio</span>
              </div>
              {signal?.fvgBull&&(
                <Badge color={T.up}>▲ {signal.fvgBull.bot.toFixed(0)}–{signal.fvgBull.top.toFixed(0)}</Badge>
              )}
              {signal?.fvgBear&&(
                <Badge color={T.down}>▼ {signal.fvgBear.bot.toFixed(0)}–{signal.fvgBear.top.toFixed(0)}</Badge>
              )}
              {!signal?.fvgBull&&!signal?.fvgBear&&(
                <Badge color={T.muted}>- NONE</Badge>
              )}
            </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"5px 0",borderBottom:`1px solid ${T.border}`}}>
              <div>
                <span style={{fontFamily:MONO,fontSize:10,color:T.muted,fontWeight:700,minWidth:28}}>PDH</span>
                <span style={{fontFamily:SANS,fontSize:8,color:T.dim,marginLeft:5}}>Liquidez</span>
              </div>
              <div style={{display:"flex",gap:4}}>
                {signal?.levels?.tpSource==="session"&&(
                  <span style={{fontFamily:MONO,fontSize:8,color:T.accent}}>TP aquí</span>
                )}
