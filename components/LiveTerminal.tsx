...
function calcSignalScore(
  ...
  // Fuera de ventana operativa — score forzado a 0
  // Consistente con el backtest que usa sessionFilter como filtro duro
  if(session==="WEEKEND"||session==="CLOSED")return 0;
  let s=0;
  if(htf!=="WAIT")s+=2;
...
function getSession():Session{
  ...
  // Mercado cerrado: Sábado todo el día + Domingo hasta las 22:00 UTC
  // XAU/USD en PU Prime abre el Domingo ~22:00 UTC (17:00 COL)
  if(day===6)return"WEEKEND";
  if(day===0&&u<1320)return"WEEKEND"; // 1320 = 22*60
  if(u>=480&&u<600)return"LDN";
  if(u>=870&&u<990)return"NY";
  return"CLOSED";
}
...
function getNextSession():{label:string;mins:number}{
  ...
  const u=now.getUTCHours()*60+now.getUTCMinutes();
  const DAY=1440;
  // Fin de semana → próximo lunes LDN (08:00 UTC)
  if(day===6){const minsToMon=(DAY-u)+DAY+480;return{label:"LDN Lunes",mins:minsToMon};}
  if(day===0&&u<1320){const minsToMon=(DAY-u)+480;return{label:"LDN Lunes",mins:minsToMon};}
  // Días de semana normales
  if(u<480)return{label:"LDN Open",mins:480-u};
  if(u>=600&&u<870)return{label:"NY Open",mins:870-u};
  const minsToLdn=u>=990?DAY-u+480:480-u;
