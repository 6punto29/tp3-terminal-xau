...
async function runSignalEmittedPipeline(deps: SignalEmittedPipelineDeps): Promise<void> {
  ...
  try {
    ...
    for (const sig of openSignals) {
      ...
      // ── Cálculo de nuevos MAE/MFE ──
      // Si no hay valor previo, inicializamos al livePrice (en la primera
      // pasada después de capturar, livePrice ≈ entry_price así que el MAE
      // y MFE arrancan cerca del entry).
      const currMae = sig.mae_price ?? livePrice;
      const currMfe = sig.mfe_price ?? livePrice;
      const newMae  = isLong ? Math.min(currMae, livePrice) : Math.max(currMae, livePrice);
      const newMfe  = isLong ? Math.max(currMfe, livePrice) : Math.min(currMfe, livePrice);
      ...
      // ── ¿La signal debe cerrarse? ──
      const outcome = checkSignalEmittedOutcome({
