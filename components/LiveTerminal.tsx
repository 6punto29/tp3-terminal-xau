"use client";
// components/LiveTerminal.tsx — v3.0
// · Estructura fractal HH/HL · LH/LL
// · Bloque MT5: entrada, SL y TP exactos listos para copiar
// · D1 Bias — sesgo del timeframe diario
// · ATR visible — volatilidad real por vela
// · Toggle manual de noticias (fix hasNews hardcodeado)
// · Score actualizado con estructura
...
import type { Candle, PriceStructure, StructureLevels } from "@/lib/engine/types";
...
function calcSignalScore(
  ...
  session:Session,structure:PriceStructure
):number{
  let s=0;
  if(htf!=="WAIT")s+=2;
...
function getNextSession():{label:string;mins:number}{
  ...
  const mins=u>=990?DAY-u+480:480-u;
  return{label:"LDN mañana",mins};
}
...
function MT5Block({signal,score,price}:MT5BlockProps){
  ...
  return(
    <Card style={{marginBottom:4,padding:"8px 10px",border:`1px solid ${color}40`,background:`${color}08`}}>
      ...
      {/* Los 3 precios */}
      {[
        {lbl:"ENTRADA",val:entry,color:T.gold,hint:"precio market"},
        {lbl:"SL",     val:sl,   color:T.down,hint:slPct},
        {lbl:"TP",     val:tp,   color:T.up,  hint:tpPct},
...
function SessionBanner({session}:{session:Session}){
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(()=>{setClock(getColTime());if(session==="CLOSED")setNext(getNextSession());},60000);
...
export default function LiveTerminal({userId,price,connected}:{userId:string;price:number;connected:boolean}){
  ...
  const signalScore=signal
    ?calcSignalScore(signal.htf,signal.mtf,signal.m15,signal.ltf,signal.rsi,signal.ema200,price,session,signal.structure)
    :0;
  ...
  useEffect(()=>{
    async function load(){
      ...
      try{
        ...
        const v=getLiveVerdict(htfInd,mtfInd,htfC.length-1,mtfC.length-1,price);
        ...
        <div className="tp3-sidebar"
          ...
          <Card style={{marginBottom:6}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              ...
              <div>
                <span style={{fontFamily:MONO,fontSize:10,color:T.muted,fontWeight:700,minWidth:28}}>D1</span>
                <span style={{fontFamily:SANS,fontSize:8,color:T.dim,marginLeft:5}}>Sesgo diario</span>
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"5px 0",borderBottom:`1px solid ${T.border}`}}>
              <div>
                <span style={{fontFamily:MONO,fontSize:10,color:T.muted,fontWeight:700,minWidth:28}}>EST</span>
                <span style={{fontFamily:SANS,fontSize:8,color:T.dim,marginLeft:5}}>Estructura</span>
              </div>
            ...
            <div style={{marginTop:7,paddingTop:6,borderTop:`1px solid ${T.border}`}}>
              {[
                ...
                <div key={label} style={{display:"flex",alignItems:"center",gap:4,marginBottom:4}}>
                  <span style={{fontFamily:SANS,fontSize:8,color:T.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em",minWidth:16}}>{label}</span>
                  <div style={{display:"flex",flexWrap:"wrap",gap:2}}>
                    {opts.map(v=>(
                      <button key={v} onClick={()=>set(v)} style={{
                        padding:"2px 4px",borderRadius:4,cursor:"pointer",fontFamily:MONO,fontSize:8,fontWeight:700,
