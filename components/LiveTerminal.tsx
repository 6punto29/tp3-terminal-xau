...
async function runSignalEmittedPipeline(deps: SignalEmittedPipelineDeps): Promise<void> {
  const { verdict, m15Sig, ltfSig, d1Bias, h4Bias, atr, htfTf, sessionTag, hasNews, livePrice, seenKeysRef } = deps;
  ...
  try {
    ...
    if (!Array.isArray(openSignals) || openSignals.length === 0) return;
