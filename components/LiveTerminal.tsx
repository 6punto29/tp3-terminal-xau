      ...
      {[
        ...
        <div key={lbl} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
          ...
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontFamily:MONO,fontSize:8,fontWeight:(lbl==="SL"||lbl==="TP")?700:400,color:c}}>{hint}</span>
...
export default function LiveTerminal({userId,price,connected,notifPerm,notifEnabled}:{userId:string;price:number;connected:boolean;notifPerm:"default"|"granted"|"denied";notifEnabled:boolean}){
  ...
  // 04/06/26: state lifted a page.tsx — notifPerm y requestNotif vienen como props.
  // Razón: el toggle de alertas vive ahora en el topbar (no en el right panel).
  // La lógica de disparo se queda acá porque depende de signal/signalScore/price.
  const lastVerdictRef=useRef<Verdict>("ESPERAR");
  const cooldownLongRef =useRef<number>(0);
  const cooldownShortRef=useRef<number>(0);

  // ── MOBILE LAYOUT (Auditoría responsive 03/06/26) ──────────────────────────
  // Tab activo en mobile (<700px): "veredicto" (default), "analisis", "operar".
  // En desktop/tablet el state existe pero CSS muestra todos los paneles igual
  // que antes — este state SOLO afecta el render en <700px vía data-tab + CSS.
  // Patrón híbrido validado por neurocog (chunking), TradingView (thumb-zone),
  // Airbnb (3-5 tabs, 40% más rápido vs hamburger menu).
  const[mobileTab,setMobileTab]=useState<"veredicto"|"analisis"|"operar">("veredicto");
  ...
  // Detector al cargar y requestNotif: movidos a page.tsx el 04/06/26.
  // (Acá se reciben como prop)

  // Detector de transición + disparador
  useEffect(()=>{
    if(typeof window==="undefined"||!("Notification" in window))return;
    // Permiso del navegador Y toggle local del usuario deben estar ON.
    // notifEnabled es el toggle "soft" desde el ícono de la campana en topbar.
  ...
  // Sticky verdict compacto usado SOLO en mobile <700px.
  // Composición JSX con colores estrictos por elemento (coherencia con VerdictCard).
  // ENTRAR LONG/SHORT con su color · FUERTE/MODERADA con su color · score con su color ·
  // estructura con su color · precio dorado.
  const stickyVerdictColor =
    !signal ? T.muted :
    signal.verdict==="ENTRAR LONG" ? T.up :
    signal.verdict==="ENTRAR SHORT" ? T.down :
    T.muted;
  const stickyStrengthColor =
    !signal ? T.muted :
    signal.strength==="FUERTE" ? T.up :
    signal.strength==="MODERADO" ? T.wait :
    T.muted;
  const stickyStructureColor =
    !signal ? T.muted :
    signal.structure==="BULLISH" ? T.up :
    signal.structure==="BEARISH" ? T.down :
    T.muted;
  const stickyScoreColor = signal && signal.verdict!=="ESPERAR" ? T.up : T.muted;
  ...
  return(
    <>
      <style>{`
        ...
        /* Elementos solo-mobile (ocultos en desktop/tablet por defecto) */
        .tp3-mobile-verdict,.tp3-mobile-nav{display:none;}
        ...
        /* Mobile <700px: sticky verdict compacto + bottom nav + un panel por tab.
           Layout flex (el container padre en page.tsx ya es flex-column):
           topbar (page.tsx) → sticky verdict → tp3-root (flex:1, scroll interno) → bottom nav.
           Sin position:fixed para evitar solapamiento con topbar.
           Fix 04/06/26: tp3-root pasa de grid a flex-column en mobile. El grid
           stackeaba sidebar/main/right en la misma celda, lo que escondía
           sidebar (Fuentes+MTF) cuando también debía mostrarse right-checklist
           en el tab Análisis. Con flex-column los visibles apilan en vertical
           y el scroll vive en el container, no en cada panel. */
        @media(max-width:700px){
          .tp3-mobile-verdict{display:block;background:${T.bg};border-bottom:1px solid ${T.border};padding:10px 12px;font-family:${MONO};font-size:11px;font-weight:700;text-align:center;letter-spacing:0.3px;flex-shrink:0;}
          .tp3-mobile-nav{display:grid;grid-template-columns:1fr 1fr 1fr;background:${T.bg};border-top:1px solid ${T.border};padding-bottom:env(safe-area-inset-bottom);flex-shrink:0;}
          .tp3-mobile-nav button{padding:10px 4px 8px;background:transparent;border:none;cursor:pointer;font-family:${SANS};font-size:9px;font-weight:700;color:${T.muted};display:flex;flex-direction:column;align-items:center;gap:2px;transition:color 0.15s,background 0.15s;}
          .tp3-mobile-nav button.active{color:${T.gold};background:rgba(212,175,55,0.08);}
          .tp3-mobile-nav .nav-ico{font-size:16px;line-height:1;}
          .tp3-root{display:flex!important;flex-direction:column;grid-template-columns:none;grid-template-rows:none;flex:1;min-height:0;overflow-y:auto;}
          .tp3-sidebar,.tp3-main,.tp3-right{grid-column:auto;grid-row:auto;overflow-y:visible!important;overflow:visible!important;height:auto!important;max-height:none;flex-shrink:0;border-left:none!important;border-right:none!important;}
          .tp3-root[data-tab="veredicto"] .tp3-sidebar{display:none!important;}
          .tp3-root[data-tab="veredicto"] .tp3-right{display:none!important;}
          .tp3-root[data-tab="analisis"] .tp3-main{display:none!important;}
          .tp3-root[data-tab="analisis"] .tp3-right-form{display:none!important;}
          .tp3-root[data-tab="operar"] .tp3-sidebar{display:none!important;}
          .tp3-root[data-tab="operar"] .tp3-main{display:none!important;}
          .tp3-root[data-tab="operar"] .tp3-right-checklist{display:none!important;}
        }
      `}</style>

      {/* Sticky verdict compacto — SOLO mobile (CSS lo oculta en desktop).
          Colores estrictos por elemento, coherente con VerdictCard. */}
      <div className="tp3-mobile-verdict">
        {!signal ? (
          <span style={{color:T.muted}}>Cargando...</span>
        ) : signal.verdict==="ESPERAR" ? (
          <>
            <span style={{color:T.muted}}>ESPERAR</span>
            <span style={{color:T.dim,margin:"0 6px"}}>·</span>
            <span style={{color:stickyScoreColor}}>{signalScore}/10</span>
            <span style={{color:T.dim,margin:"0 6px"}}>·</span>
            <span style={{color:stickyStructureColor}}>{structureLabel}</span>
            {price>0 && <>
              <span style={{color:T.dim,margin:"0 6px"}}>·</span>
              <span style={{color:T.gold}}>${price.toFixed(2)}</span>
            </>}
          </>
        ) : (
          <>
            <span style={{color:stickyVerdictColor}}>{signal.verdict}</span>
            <span style={{color:T.dim,margin:"0 6px"}}>·</span>
            <span style={{color:stickyStrengthColor}}>{signal.strength}</span>
            <span style={{color:T.dim,margin:"0 6px"}}>·</span>
            <span style={{color:stickyScoreColor}}>{signalScore}/10</span>
            <span style={{color:T.dim,margin:"0 6px"}}>·</span>
            <span style={{color:stickyStructureColor}}>{structureLabel}</span>
      ...
      <div className="tp3-root" data-tab={mobileTab} style={{fontFamily:SANS,color:T.text,background:T.bg}}>
        ...
        <div className="tp3-right"
          ...
          {/* Push notif: el toggle vive ahora en el topbar (page.tsx).
              Aquí queda solo Checklist + OperationForm.
              En mobile las clases tp3-right-checklist y tp3-right-form
              se ocultan condicionalmente según el tab activo. */}
          <div className="tp3-right-checklist">
            <Checklist
          ...
          <div className="tp3-right-form" style={{flex:1,display:"flex",flexDirection:"column",minHeight:0}}>
            <OperationForm
      ...
      {/* Bottom navigation — SOLO mobile (CSS lo oculta en desktop) */}
      <div className="tp3-mobile-nav">
        <button
          className={mobileTab==="veredicto"?"active":""}
          onClick={()=>setMobileTab("veredicto")}
          aria-label="Tab Veredicto"
        >
          <span className="nav-ico">◎</span>
          <span>Veredicto</span>
        </button>
        <button
          className={mobileTab==="analisis"?"active":""}
          onClick={()=>setMobileTab("analisis")}
          aria-label="Tab Análisis"
        >
          <span className="nav-ico">▦</span>
          <span>Análisis</span>
        </button>
        <button
          className={mobileTab==="operar"?"active":""}
          onClick={()=>setMobileTab("operar")}
          aria-label="Tab Registro"
        >
          <span className="nav-ico">✎</span>
          <span>Registro</span>
        </button>
