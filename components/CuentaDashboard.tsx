...
export default function CuentaDashboard({userId}:{userId:string}){
  ...
  // Avg R en ganadoras / perdedoras (para R:R Real)
  ...
  // Sharpe simplificado sobre R (requiere ≥20 cerradas)
  let sharpe:number|null = null;
  if(closedAll.length >= 20){
    const meanR = closedAll.reduce((a,o)=>a+o.r,0)/closedAll.length;
    const varR = closedAll.reduce((a,o)=>a+Math.pow(o.r-meanR,2),0)/closedAll.length;
    const sdR = Math.sqrt(varR);
    sharpe = sdR>0 ? meanR/sdR : null;
  }
  ...
  // Detalle de Performance — los titulares (P&L/Expectancy/Aciertos/R:R) se arman aparte en el render
  const stats=[
    ...
    {l:"Sharpe Ratio",  v:sharpe!=null?fmtNum(sharpe):closedAll.length<20?`-- (n<20)`:"--", c:sharpe==null?T.muted:sharpe>=1?T.up:T.wait},
  ...
  function calcShadowStats(rows: ShadowTradeRow[]) {
    ...
    // P&L $ teórico: solo suma WIN/LOSS con snapshot capital + risk + sl_pct válidos.
    // EXPIRED se EXCLUYE (no tocó TP ni SL en 24h, su pnl_pct es arbitrario — el
    // precio al cierre de 24h, no un resultado del sistema). Decisión 11/06/26
    // alineada con el Triple Barrier Method de López de Prado: el time-exit es
    // categoría neutral separada de WIN/LOSS. Mismo criterio que el P&L del motor.
    // R = pnl_pct / sl_pct; dollar risk = capital × (risk_pct/100); P&L $ = R × dollar risk.
    // Filas pre-snapshot (capital/risk NULL) se ignoran. Si ninguna aplica, devuelve null.
    const withSnap = closed.filter(t => (t.status === "WIN" || t.status === "LOSS") && t.capital_at_signal != null && t.risk_pct_at_signal != null && t.sl_pct > 0 && t.pnl_pct != null);
    const pnlDollar = withSnap.length > 0
      ? withSnap.reduce((acc, t) => {
          const r = (t.pnl_pct as number) / t.sl_pct;
          const dollarRisk = (t.capital_at_signal as number) * ((t.risk_pct_at_signal as number) / 100);
          return acc + (r * dollarRisk);
        }, 0)
      : null;
    return { total:rows.length, closed:closed.length, open:rows.length-closed.length,
             wins, losses, expired, ofrr, wr, pf, avgPnl, bestPnl, worstPnl, pnlDollar };
  }
  ...
  const copyStats = async()=>{
    ...
    const text = [
      ...
      `  Sharpe Ratio:   ${sharpe!=null?fmtNum(sharpe):closedAll.length<20?`-- (n<20, requiere ≥20 cerradas)`:"--"}`,
      ...
      <div style={{maxWidth:1100,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        <Card>
          ...
          {signalsLoading?(
            ...
            <>
              {/* Titulares — los 3 números que dicen si el sistema funciona */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div style={{background:T.s2,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{fontFamily:MONO,fontSize:26,fontWeight:700,lineHeight:1,color:snapCount===0?T.muted:pnlTotal>=0?T.up:T.down}}>{snapCount>0?fmtMoney(pnlTotal):"--"}</div>
                </div>
                <div style={{background:T.s2,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{fontFamily:MONO,fontSize:26,fontWeight:700,lineHeight:1,color:expectancy==null?T.muted:expectancy>0?T.up:T.down}}>{expectancy==null?"--":`${expectancy>=0?"+":""}${expectancy.toFixed(2)}R`}</div>
                </div>
                <div style={{gridColumn:"1 / -1",background:T.s2,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:18,flexWrap:"wrap"}}>
                  <div style={{display:"flex",flexDirection:"column"}}>
                    <div style={{fontFamily:SANS,fontSize:9,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:T.muted,marginBottom:5}}>Aciertos</div>
                    <div style={{fontFamily:MONO,fontSize:24,fontWeight:700,lineHeight:1,color:T.text}}>{totalContados>0?`${wr}%`:"--"}</div>
                  </div>
                  <div style={{width:1,alignSelf:"stretch",background:T.border2}}></div>
                  <div style={{display:"flex",flexDirection:"column"}}>
                    <div style={{fontFamily:SANS,fontSize:9,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:T.muted,marginBottom:5}}>R:R Real</div>
                  ...
                  {rrReal!=null && totalContados>0 && (
                    <div style={{fontFamily:SANS,fontSize:10,color:T.muted,lineHeight:1.4,flex:1,minWidth:160}}>
              ...
              {/* Detalle — el resto, más chico */}
              <div style={{fontFamily:SANS,fontSize:9,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:T.dim,margin:"10px 0 6px"}}>Detalle</div>
                ...
                <ScrollX minWidth={680}>
                  <div style={{display:"flex",flexDirection:"column",gap:3}}>
                    <div style={{display:"grid",gridTemplateColumns:"160px 60px 60px 60px 70px 70px 70px 80px",gap:6,
                      ...
                      <span>Avg PnL</span><span>Best</span><span>Worst</span><span>P&L $</span>
                    </div>
                    {[
