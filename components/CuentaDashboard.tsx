...
export default function CuentaDashboard({userId}:{userId:string}){
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Performance ahora se calcula desde signals_emitted con snapshot capital + riesgo
  // (Opción B, 10/06/26). Las ops manuales xau_usd siguen en BD pero ya no alimentan
  // estas métricas — el motor se valida por su universo completo de señales (WIN/LOSS).
  ...
  type PerfOp = { id: string; pnl: number; resultado: "TP" | "SL" };
  const closed: PerfOp[] = signals
  ...
  const winOps  = closed.filter(o => o.resultado==="TP");
  const lossOps = closed.filter(o => o.resultado==="SL");
  ...
  for(let i=closed.length-1;i>=0;i--){
    const o = closed[i];
    const isWin = o.resultado==="TP";
    const isLoss = o.resultado==="SL";
  ...
  const stats=[
    ...
    {l:"Operaciones",   v:`${signals.length}`,                      c:T.text},
    {l:"Pendientes",    v:`${signals.filter(s=>s.status==="OPEN").length}`, c:T.muted},
  ];
  ...
  const copyStats = async()=>{
    ...
    const text = [
      ...
      `  Operaciones:  ${signals.length}  ·  Pendientes: ${signals.filter(s=>s.status==="OPEN").length}`,
      "━━━━━━━━━━━━━━━━━━━━━━━━━━",
    ].join("\n");
