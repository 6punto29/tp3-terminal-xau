...
export default function CuentaDashboard({userId}:{userId:string}){
  ...
  const[tfFilter,setTfFilter]=useState<"1h"|"4h"|"all">("1h");             // fix #1: separa Performance por temporalidad (htf_tf)
  ...
  // ── FILTRO POR TEMPORALIDAD (fix #1, 13/06/26) ──────────────────────────
  // Separa la Performance por htf_tf. "1h" (default) = validacion intradia pura;
  // "4h" = capturas del toggle (referencia); "all" = universo mezclado (como antes).
  // Solo afecta las metricas de Performance. NO toca captura, motor ni BD.
  const signalsForPerf = signals.filter(s => tfFilter === "all" || s.htf_tf === tfFilter);
  ...
  const closedAll: MotorOp[] = signalsForPerf
  ...
  const closedSnap: DollarOp[] = signalsForPerf
  ...
  // Glosario de metricas (nombres estandar de mercado) -- bloque 2 de la barra Guia
  const glosario=[
    {g:"Lo básico",items:[
      {n:"R",tag:"la unidad",d:<>La unidad de todo. 1R = lo que arriesgás por señal (tu 1% = $100). Una señal de <span style={{color:T.up,fontWeight:600}}>+2R</span> ganó $200; una de <span style={{color:T.down,fontWeight:600}}>-1R</span> perdió $100. Todo se mide en R para comparar parejo.</>},
      {n:"Win Rate",tag:"% que ganan",d:<>De cada 100 señales, cuántas ganan. Ojo: no dice <span style={{color:T.text,fontWeight:600}}>cuánto</span> ganan — podés acertar poco y aun así ganar plata.</>},
      {n:"Payoff Ratio",tag:"ganadora vs perdedora",d:<>Cuánto pesa en promedio una ganadora frente a una perdedora. <span style={{color:T.text,fontWeight:600}}>2.62×</span> = cada ganadora vale como 2.62 perdedoras.</>},
      {n:"Expectancy",tag:"promedio por señal",d:<>Lo que te deja en promedio cada señal, en R. <span style={{color:T.text,fontWeight:600}}>+0.13R</span> = de promedio ganás 0.13R por señal.</>},
      {n:"Profit Factor",tag:"ganado / perdido",d:<>Dólares ganados ÷ dólares perdidos. Arriba de 1 = ganás. <span style={{color:T.text,fontWeight:600}}>1.19</span> = por cada $1 perdido, entran $1.19.</>},
    ]},
    {g:"Calidad y riesgo",items:[
      {n:"SQN",tag:"nota de calidad",d:<>La nota de calidad del sistema: junta el edge, la consistencia y el tamaño de muestra. Más alto = mejor y más confiable. Abajo de 1.5 = todavía sin probar.</>},
      {n:"Std Dev (R)",tag:"dispersión",d:<>Qué tan dispersos están los resultados. Alto = muy variable (unas muy ganadoras, otras muy perdedoras).</>},
      {n:"Sharpe",tag:"ajustado por riesgo",d:<>Rendimiento ajustado por riesgo. Castiga <span style={{color:T.text,fontWeight:600}}>toda</span> la variación, incluso las ganadoras grandes.</>},
      {n:"Sortino",tag:"riesgo solo a la baja",d:<>Como el Sharpe, pero solo castiga las caídas (no las ganadoras grandes). Más justo para un sistema como el tuyo.</>},
      {n:"Recovery Factor",tag:"neto / peor caída",d:<>Ganancia neta ÷ la peor caída. Qué tan bien te levantás de los baches.</>},
      {n:"Avg MAE (win)",tag:"aguante de ganadoras",d:<>Cuánto se fueron en contra las ganadoras antes de dar vuelta a favor. Bajo = casi no retroceden (sirve para calibrar el SL).</>},
    ]},
    {g:"Operación",items:[
      {n:"Max DD",tag:"peor caída en $",d:<>La caída más grande de tu dinero desde un pico. La peor racha mala, en dólares.</>},
      {n:"Streak",tag:"racha actual",d:<>La racha actual de resultados seguidos. <span style={{color:T.up,fontWeight:600}}>✓</span> = ganadoras seguidas, <span style={{color:T.down,fontWeight:600}}>✗</span> = perdedoras seguidas.</>},
    ]},
  ];
      ...
      <div style={{maxWidth:1200,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div style={{position:"relative"}}>
          <button
            ...
            <span style={{color:T.gold,fontWeight:700}}>Guía</span>
            <span>cómo decide el motor · glosario de métricas</span>
          ...
          {reglasOpen&&(
            <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,zIndex:100,
              background:T.s1,border:`1px solid ${T.border}`,borderRadius:7,padding:"4px 12px 12px",
              boxShadow:"0 8px 24px rgba(0,0,0,0.4)",maxHeight:"70dvh",overflowY:"auto"}}>

              {/* Bloque 1 -- Como decide el motor (lo que ya existia) */}
              <div style={{padding:"10px 0 12px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                  <span style={{width:7,height:7,borderRadius:"50%",background:T.accent,flexShrink:0}}></span>
                  <span style={{fontFamily:SANS,fontSize:12,fontWeight:700,color:T.text}}>Cómo decide el motor</span>
                </div>
              ...
              {/* Bloque 2 -- Glosario (acordeon desplegable) */}
              <div style={{padding:"12px 0 2px",borderTop:`1px solid ${T.border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                  <span style={{width:7,height:7,borderRadius:"50%",background:T.gold,flexShrink:0}}></span>
                  <span style={{fontFamily:SANS,fontSize:12,fontWeight:700,color:T.text}}>Glosario</span>
                  <span style={{fontFamily:SANS,fontSize:10,color:T.muted}}>— tocá un término para ver qué significa</span>
                </div>
                {glosario.map((grp)=>(
                  <div key={grp.g}>
                    <div style={{fontFamily:SANS,fontSize:9,fontWeight:700,letterSpacing:"0.10em",textTransform:"uppercase",color:T.dim,margin:"10px 0 4px"}}>{grp.g}</div>
                    {grp.items.map((it)=>{
                      const gOpen=openGloss[it.n]===true;
                      return(
                        <div key={it.n} style={{borderBottom:`1px solid ${T.border}`}}>
                          <div onClick={()=>setOpenGloss(p=>({...p,[it.n]:!p[it.n]}))}
                            style={{display:"flex",alignItems:"center",gap:9,padding:"8px 4px",cursor:"pointer",userSelect:"none"}}>
                            <span style={{color:gOpen?T.accent:T.muted,fontSize:9,width:9,flexShrink:0,display:"inline-block",transform:gOpen?"rotate(90deg)":"none",transition:"transform 0.2s ease"}}>▶</span>
                            <span style={{fontFamily:MONO,fontSize:12,fontWeight:700,color:T.text}}>{it.n}</span>
                            <span style={{marginLeft:"auto",fontFamily:SANS,fontSize:11,color:T.muted,fontStyle:"italic",opacity:gOpen?0.45:1,textAlign:"right"}}>{it.tag}</span>
                          </div>
                          {gOpen&&(
                            <div style={{padding:"1px 4px 10px 22px",fontFamily:SANS,fontSize:11,color:T.muted,lineHeight:1.55}}>{it.d}</div>
                          )}
                        </div>
                      );
                    })}
        ...
        <Card>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10,gap:8,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:3,background:T.bg,padding:3,borderRadius:8,border:`1px solid ${T.border}`}}>
                {(["1h","4h","all"] as const).map(tf=>(
                  <div key={tf} onClick={()=>setTfFilter(tf)} style={{
                    fontFamily:SANS,fontSize:10,fontWeight:tfFilter===tf?700:600,
                    padding:"4px 11px",borderRadius:6,cursor:"pointer",userSelect:"none",
                    background:tfFilter===tf?T.s3:"transparent",
                    color:tfFilter===tf?T.text:T.muted,
                    border:`1px solid ${tfFilter===tf?T.accent:"transparent"}`,
                    transition:"all 0.12s"
                  }}>{tf==="1h"?"1H":tf==="4h"?"4H":"Todas"}</div>
                ))}
              </div>
              <span style={{fontFamily:MONO,fontSize:9,color:T.dim}}>{signalsForPerf.length} señales · {signalsForPerf.filter(s=>s.status==="OPEN").length} abiertas</span>
            </div>
            <button
              onClick={copyStats}
