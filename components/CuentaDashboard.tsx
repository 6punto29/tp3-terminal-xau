          ...
          <div style={{display:"flex",flexDirection:"column",gap:2}}>
            {reglas.map(({icn,txt},i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:7,
                padding:"4px 8px",borderRadius:5,background:T.s2}}>
                <span style={{fontSize:11,flexShrink:0}}>{icn}</span>
                <span style={{fontFamily:SANS,fontSize:11,color:T.muted,lineHeight:1.4}}>{txt}</span>
              </div>
            ))}
          </div>
          <div style={{marginTop:7,padding:"5px 8px",borderRadius:5,
            background:`${T.gold}12`,border:`1px solid ${T.gold}30`,
            fontFamily:SANS,fontSize:10,color:T.gold,lineHeight:1.5}}>
