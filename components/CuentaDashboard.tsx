...
export default function CuentaDashboard({userId}:{userId:string}){
  ...
  // Grilla 4×4 = 16 métricas. Las del MOTOR (universo completo) van en % y R;
  // las en DÓLARES vienen del subconjunto con snapshot (snapCount señales).
  const stats=[
    // Fila 1 — Rendimiento del motor (universo completo)
    ...
    {l:"Expectancy",    v:expectancy==null?"--":`${expectancy>=0?"+":""}${expectancy.toFixed(2)}R`, c:expectancy==null?T.muted:expectancy>0?T.up:T.down},
    {l:`P&L $ (${snapCount})`, v:snapCount>0?fmtMoney(pnlTotal):"--", c:snapCount===0?T.muted:pnlTotal>=0?T.up:T.down},
    // Fila 2 — Detalle de operaciones (motor)
    ...
    {l:"Señales",       v:`${signals.length}`,                      c:T.text},
    {l:"Abiertas",      v:`${signals.filter(s=>s.status==="OPEN").length}`, c:T.muted},
  ];
  ...
  return(
    <div style={{background:T.bg,minHeight:"100%",padding:"16px 16px calc(16px + env(safe-area-inset-bottom)) 16px",overflowY:"auto"}}>
      <div style={{maxWidth:1100,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {/* Stats grid — Performance XAU/USD (reordenado 09/06/26: ahora va antes del Historial, después de Shadow Trading) */}
        <Card>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10,gap:8,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
