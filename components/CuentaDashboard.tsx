...
export default function CuentaDashboard({userId}:{userId:string}){
  ...
  const stats=[
    ...
    {l:"P&L Total",  v:`${pnlTotal>=0?"+":""}${pnlTotal.toFixed(2)}%`, c:pnlTotal>=0?T.up:T.down},
    {l:"Ganadoras",  v:`${wins}`,              c:T.up},
    {l:"Perdedoras", v:`${losses}`,            c:T.down},
    {l:"Max DD",     v:`-${maxDD.toFixed(2)}%`,c:T.wait},
  ...
  return(
    <div style={{background:T.bg,minHeight:"100%",padding:"16px",overflowY:"auto"}}>
      <div style={{maxWidth:1100,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {/* Reglas de gestión */}
        <Card>
          <SecTitle>Reglas de Gestión</SecTitle>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {reglas.map(({icn,txt},i)=>(
              <div key={i} style={{display:"flex",alignItems:"flex-start",gap:10,
                padding:"8px 12px",borderRadius:7,background:T.s2,
                border:`1px solid ${T.border}`}}>
                <span style={{fontSize:14,flexShrink:0,marginTop:1}}>{icn}</span>
                <span style={{fontFamily:SANS,fontSize:12,color:T.text,lineHeight:1.5}}>{txt}</span>
              </div>
            ))}
          </div>
          <div style={{marginTop:10,padding:"8px 12px",borderRadius:7,
            background:`${T.gold}12`,border:`1px solid ${T.gold}30`,
            fontFamily:SANS,fontSize:11,color:T.gold,lineHeight:1.6}}>
            💡 El sistema tiene EV +1.55R con WR 51% sobre 81 señales reales. La disciplina en estas reglas es lo que separa el backtest del resultado real.
          </div>
        </Card>
        ...
        <Card>
          ...
          {loading?(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              {/* Header */}
              <div style={{display:"grid",gridTemplateColumns:"60px 80px 1fr 90px 90px 80px 80px",
                ...
                <span>SL</span><span>TP</span><span>Resultado</span><span>P&L</span>
              </div>
              {ops.map(op=>(
                <div key={op.id} style={{display:"grid",
                  gridTemplateColumns:"60px 80px 1fr 90px 90px 80px 80px",
                  ...
                  <span style={{fontFamily:MONO,fontSize:10,color:T.muted}}>{op.fecha}</span>
                  ...
                  <span style={{fontFamily:MONO,fontSize:11,fontWeight:700,
                    ...
                    {op.pnl!=null?`${op.pnl>=0?"+":""}${op.pnl.toFixed(2)}%`:"--"}
                  </span>
                </div>
              ))}
            </div>
