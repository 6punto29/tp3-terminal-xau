...
export default function CuentaDashboard({userId}:{userId:string}){
  ...
  function calcShadowStats(rows: ShadowTradeRow[]) {
    ...
    // P&L $ teórico: solo suma closed con snapshot capital + risk + sl_pct válidos.
    // R = pnl_pct / sl_pct; dollar risk = capital × (risk_pct/100); P&L $ = R × dollar risk.
    // Filas pre-snapshot (capital/risk NULL) se ignoran. Si ninguna tiene snapshot, devuelve null.
    const withSnap = closed.filter(t => t.capital_at_signal != null && t.risk_pct_at_signal != null && t.sl_pct > 0 && t.pnl_pct != null);
