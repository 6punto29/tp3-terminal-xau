...
interface ResumenContextValue {
  ...
  // Section 3 — Comparar R:R · Mejor config automática
  section3: { rows: RRRow[]; baseLabel: string } | null;
  setSection3: (v: { rows: RRRow[]; baseLabel: string } | null) => void;
  // Section 4 — Comparar Ratio + ATR (12 combos)
  section4: { rows: RatioAtrRow[] } | null;
  setSection4: (v: { rows: RatioAtrRow[] } | null) => void;
}
...
function Section3() {
  ...
  useEffect(() => {
    ...
    if (rows && rows.length > 0) {
      ...
      resumen.setSection3({ rows: top5, baseLabel });
    }
  }, [rows, resumen]);
...
function ResumenParaClaude() {
  ...
  if (!s3Ready) missing.push("Comparar R:R");
  if (!s4Ready) missing.push("Comparar Ratio + ATR");
  ...
  const copyAll = useCallback(async () => {
    ...
    // ── SECCIÓN 3 ────────────────────────────────────────────────────────────
    const s3Header =
      `📊 SECTION 3 — Comparar R:R con mejor config\n` +
      `Base: ${section3.baseLabel}`;
  ...
  return (
    <BtCard>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 10 }}>
        ...
        <StatusRow ok={s3Ready} label="Comparar R:R · Mejor config automática (Sección 3)" />
        <StatusRow ok={s4Ready} label="Comparar Ratio + ATR (Sección 4)" />
      </div>
      {!allReady && (
        <div style={{
