...
interface ResumenContextValue {
  ...
  // Default true = score filter activo (post-Fix #2, backtest = live).
  // Si false = backtest sin score filter (= comportamiento pre-Fix #2, 39 señales).
...
// ─────────────────────────────────────────────────────────────────────────────
//
// Propósito: validar A/B la hipótesis "el score filter está descartando las
// mejores señales". Permite correr el MISMO período con y sin filter.
//
// ON  (default) = comportamiento post-Fix #2 (backtest = live, 13 señales aprox)
// OFF           = comportamiento pre-Fix #2 (sin score filter, 39 señales aprox)
  ...
  return (
    <BtCard>
      ...
      <div style={{
        ...
        {applyScoreFilter
          ? "Activo: backtest aplica el mismo filtro que el LIVE (~13 señales/mes)."
          : "Desactivado: backtest acepta todas las señales que pasen HTF+MTF (~39 señales/mes)."}
      </div>
...
function ResumenParaClaude() {
  ...
  const copyAll = useCallback(async () => {
    ...
    const filterTag = ctx.applyScoreFilter
      ? "Score filter: ON (igual que live)"
      : "Score filter: OFF (sin filtro · pre-Fix #2)";
    const txt =
      `${SEP}\n` +
