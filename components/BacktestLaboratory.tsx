...
import { simulateSignals, summarize } from "@/lib/engine/simulator";
...
// Páginas de histórico por timeframe (cada página = 1500 velas de Binance).
// Calibrado para que TODOS los timeframes cubran ~5-6 meses. El 5m necesita
// muchas más páginas que el 4h porque tiene 48× más velas por día.
const PAGES_BY_TF: Record<string, number> = {
  "5m":  30,  // ~5 meses
  "15m": 11,  // ~5.5 meses
  "30m": 6,   // ~6 meses
  "1h":  4,   // ~8 meses
...
// Mapeo HTF → MTF para el test manual (Section 1).
const MTF_MAP: Record<string, string> = {
  "4h": "1h", "1h": "15m", "30m": "5m", "15m": "5m",
};
const ALL_CONFIGS: Pick<BacktestConfig, "htf" | "mtf" | "hold">[] = [
  ...
  { htf: "30m", mtf: "5m",  hold: 3  },
  { htf: "30m", mtf: "5m",  hold: 6  },
  { htf: "30m", mtf: "5m",  hold: 12 },
  { htf: "15m", mtf: "5m",  hold: 3  },
  { htf: "15m", mtf: "5m",  hold: 6  },
  { htf: "15m", mtf: "5m",  hold: 12 },
];
...
interface ResumenContextValue {
  ...
  // CAMBIO Toggle Score Filter (21/05/26): flag global compartido por las 4 secciones.
  // Default true = score filter activo (backtest = live, ~7 señales/mes).
  // Si false = backtest sin score filter (acepta todo HTF+MTF, cientos de señales).
  // Útil para A/B: comparar el mismo período con y sin el filtro.
  applyScoreFilter: boolean;
  setApplyScoreFilter: (v: boolean) => void;
}
...
async function fetchCandles(tf: string, signal?: AbortSignal, onProgress?: (msg: string) => void): Promise<Candle[]> {
  ...
  // Paginar hacia atrás. El nº de páginas depende del timeframe: el 5m necesita
  // ~30 páginas para llegar a 5 meses, el 4h solo 2. Ver PAGES_BY_TF.
  const maxPages = PAGES_BY_TF[tf] ?? 8;
...
async function runBacktest(cfg: BacktestConfig, signal?: AbortSignal, onProgress?: (msg: string) => void): Promise<BacktestResult> {
  ...
  // needX = ¿hay que descargar ese timeframe por separado?
  // Solo si NO coincide ni con el HTF ni con el MTF de la config (así no se
  // descarga dos veces el mismo timeframe — ej: config 15m+5m no re-baja el 5m).
  const needH4  = cfg.htf !== "4h"  && cfg.mtf !== "4h";
  const needM15 = cfg.htf !== "15m" && cfg.mtf !== "15m";
  const needLTF = cfg.htf !== "5m"  && cfg.mtf !== "5m";
  ...
  const ltfExtra   = needLTF ? await fetchCandles("5m",  signal, onProgress) : [] as Candle[];
  const h4Extra    = needH4  ? await fetchCandles("4h",  signal, onProgress) : [] as Candle[];

  // Reutiliza el fetch de HTF o MTF si ya coincide con el timeframe pedido.
  const pick = (tf: string, extra: Candle[]): Candle[] =>
    cfg.htf === tf ? htfCandles : cfg.mtf === tf ? mtfCandles : extra;
  const m15Candles = pick("15m", m15Extra);
  const h4Candles  = pick("4h",  h4Extra);
  const ltfCandles = pick("5m",  ltfExtra);
  if (signal?.aborted) throw new DOMException("Backtest cancelado", "AbortError");
  ...
  const signals = detectSignals(htfCandles, mtfCandles, htfInd, mtfInd, {
    ...
    atrMin:           cfg.atrMin,
    // FIX (24/05/26): el toggle Score Filter NO se estaba pasando al motor.
    // Por eso ON y OFF daban resultados idénticos — el filtro nunca corría.
    applyScoreFilter: cfg.applyScoreFilter,
  }, {
    m15Candles,
    ltfCandles,
    h4Candles,
  ...
  // rr NOMINAL — solo para mostrar como dato informativo (TP%/SL% de la config).
  const rr      = cfg.tpPct / cfg.slPct;
  // FIX 1 (24/05/26): el EV se LEE del summary — calculado con calcEVfromTrades
  // (promedio de R reales). Ya NO se recalcula con calcEV(wr, rr) nominal,
  // que asumía un R:R fijo que los trades estructurales no tienen.
  const ev      = summary?.ev ?? 0;
...
function CopyResultBtn({ result, sl, tp, spread }: {
  ...
  const copy = () => {
    ...
    const txt = [
      ...
      `Profit Factor: ${(s.profitFactor??0).toFixed(2)}`,
...
// FIX 2 (24/05/26): se eliminó CopyRRBtn — copiaba la config "óptima" coronada.
// El backtest ya no corona configs; el usuario lee la tabla y decide.
...
function ResultTable({
  ...
  return (
    <ScrollX minWidth={showSLTP ? 960 : 720}>
...
function Section1() {
  ...
  // Toggle Score Filter (21/05/26): leer del context global
  const resumen = useResumen();
  const applyScoreFilter = resumen?.applyScoreFilter ?? true;
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const cfg: BacktestConfig = {
        ...
        minRatio,
        atrMin,
        applyScoreFilter,   // ← Toggle Score Filter (21/05/26)
      };
  ...
  return (
    <BtCard>
      ...
      <div style={{
        ...
        <SelBtn active={htf === "30m"} onClick={() => setHtf("30m")}>HTF 30M</SelBtn>
        <SelBtn active={htf === "15m"} onClick={() => setHtf("15m")}>HTF 15M</SelBtn>
        <span style={{ width: 8 }} />
      ...
      {result && s && (
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap", alignItems:"center" }}>
          {[
            ...
            // FIX 3 (24/05/26): Profit Factor — robustez del edge.
            // >1.5 sólido (verde) · >1.2 frágil (ámbar) · <1.2 no sobrevive real (rojo).
            { lbl: "Profit Factor",
              val: `${(s.profitFactor ?? 0).toFixed(2)}`,
              c: (s.profitFactor ?? 0) >= 1.5 ? T.up
                 : (s.profitFactor ?? 0) >= 1.2 ? T.wait
                 : T.down },
...
function Section2() {
  ...
  const resumen = useResumen();
  // Toggle Score Filter (21/05/26): leer flag global del context
  const applyScoreFilter = resumen?.applyScoreFilter ?? true;
  useEffect(() => {
    if (!resumen) return;
  ...
  const run = useCallback(async () => {
    ...
    try {
        ...
        for (const ema200Filter of [false, true]) {
          ...
          try {
            ...
            const res = await runBacktest({
              ...
              applyScoreFilter,   // ← Toggle (21/05/26)
            }, ctrl.signal);
            const s = res.summary;
            if (s && s.total > 0) {
              results.push({
      ...
      if (cancelled) {
        ...
        setStat(
          top1
            ? `Listo · ${results.length} configs · top por EV: ${top1.label} · EV ${fmtPct(top1.ev, 2).replace("%","R")} · WR ${top1.wr}% (ver tabla)`
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (cancelled) {
        ...
        setStat(
          top1
            ? `Listo · ${results.length} combos · top por EV: ${top1.label} · EV ${(top1.ev>=0?"+":"")}${top1.ev.toFixed(2)}R · WR ${top1.wr}% (ver tabla)`
            : "Sin resultados — verifica conexión a Binance"
        );
      }
    } finally {
...
function Section4() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (cancelled) {
        ...
        // FIX 2/4 (24/05/26): status informativo, no corona "★ Óptimo".
        // Muestra la fila top de la tabla como referencia — el usuario decide.
        const top1 = top[0];
        setStat(
          top1
            ? `Listo · ${top.length} configs · top por EV: ${top1.label} ` +
              `SL ${top1.slPct?.toFixed(2).replace(".00","")}% TP ${top1.tpPct?.toFixed(2).replace(".00","")}% ` +
              `· EV +${top1.ev.toFixed(2)}R · WR ${top1.wr}% (ver tabla completa)`
            : "Sin resultados"
        );
      }
    } finally {
  ...
  return (
    <BtCard>
      ...
      {/* FIX 2 (24/05/26): se eliminó el bloque "★ Óptimo absoluto" que coronaba
          automáticamente la config de mayor EV. Ordenar solo por EV ignora el WR
          y el Profit Factor — es el "trap de optimización". El backtest INFORMA,
          no decide. El usuario lee la tabla completa (EV + WR + PF) y elige.
          FIX 4: se eliminó el check muerto "SL 1.5% + TP 4%" (config vieja). */}
        ...
        <>
          ...
          {best && (
            <div style={{
              ...
              background:`${T.wait}0A`,border:`1px solid ${T.wait}30`,
            }}>
              <div style={{fontFamily:SANS,fontSize:9,fontWeight:700,color:T.wait,marginBottom:4}}>
                ⚠ Cómo leer esta tabla
              </div>
              <div style={{fontFamily:SANS,fontSize:9,color:T.muted,lineHeight:1.5}}>
                La tabla está ordenada por EV, pero el EV más alto NO es
                automáticamente la mejor opción. Un EV alto con WR bajo es más
                difícil de operar (más rachas perdedoras seguidas). Compará
                EV, WR y Profit Factor juntos — y elegí la config que puedas
                sostener psicológicamente, no solo la del número más alto.
              </div>
            </div>
          )}
        </>
...
// ─────────────────────────────────────────────────────────────────────────────
// SCORE FILTER TOGGLE — Toggle global compartido entre las 4 secciones (21/05/26)
// ─────────────────────────────────────────────────────────────────────────────
//
// Propósito: validar A/B el efecto del score filter. Permite correr el MISMO
// período con y sin filter.
//
// ON  (default) = backtest aplica el filtro de score igual que el LIVE (~37
//                 señales en 5 meses, ~7/mes — datos backtest limpio 24/05/26)
// OFF           = sin filtro de score, acepta todo HTF+MTF (cientos de señales,
//                 mayoría ruido — no representa el sistema real)

function ScoreFilterToggle() {
  const ctx = useResumen();
  if (!ctx) return null;
  const { applyScoreFilter, setApplyScoreFilter } = ctx;

  const Btn = ({ on, label, value }: { on: boolean; label: string; value: boolean }) => (
    <button
      onClick={() => setApplyScoreFilter(value)}
      style={{
        flex: 1,
        fontFamily: SANS, fontSize: 11, fontWeight: 700,
        padding: "7px 12px", borderRadius: 6,
        border: `1px solid ${on ? (value ? T.up : T.wait) : T.border}`,
        background: on ? (value ? `${T.up}1A` : `${T.wait}1A`) : "transparent",
        color: on ? (value ? T.up : T.wait) : T.muted,
    ...
    <BtCard>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        ...
        <span style={{
          ...
          Score Filter (umbral 6/10)
        </span>
      </div>
      <div style={{
        fontFamily: SANS, fontSize: 9, color: T.muted, marginBottom: 8, lineHeight: 1.4,
      }}>
        {applyScoreFilter
          ? "Activo: backtest aplica el mismo filtro de score que el LIVE (~7 señales/mes)."
          : "Desactivado: backtest acepta todas las señales que pasen HTF+MTF — incluye mucho ruido (cientos de señales)."}
      </div>
      <div style={{ display: "flex", gap: 6 }}>
        <Btn on={applyScoreFilter}  value={true}  label="ON · igual que LIVE" />
        <Btn on={!applyScoreFilter} value={false} label="OFF · sin filtro" />
      </div>
      <div style={{
        marginTop: 8, fontFamily: MONO, fontSize: 9, color: T.dim, textAlign: "center",
      }}>
        Al cambiar el toggle se limpian los resultados previos para forzar nueva corrida.
      </div>
    </BtCard>
  );
}
...
function ResumenParaClaude() {
  ...
  const copyAll = useCallback(async () => {
    ...
    // BUG FIX 21/05/26: incluir r.label en cada fila — antes mostraba SL/TP idénticos
    // para configs base distintas (ej: H3 +EMA vs H12 +EMA con SL 0.50% / TP 2.50%
    // aparecían como 2 filas iguales con WR distintos sin explicación).
    const s4Header =
      `📊 SECTION 4 — Comparar R:R · top 5\n` +
      `Mejor base: ${section4.baseLabel}`;
    const s4Body = section4.rows
      .map((r) => {
        ...
        return `${r.label.padEnd(22)} · SL ${slStr.padEnd(6)} · TP ${tpStr.padEnd(6)} · R:R ${rrStr.padEnd(7)} | ${String(r.total).padStart(3)} señales | WR ${String(r.wr).padStart(2)}% | EV ${r.ev >= 0 ? "+" : ""}${r.ev.toFixed(2)}R`;
      })
      .join("\n");

    // Orden natural: Section 2 → 3 → 4 (= orden físico del laboratorio)
    const filterTag = ctx.applyScoreFilter
      ? "Score filter: ON (igual que el LIVE)"
      : "Score filter: OFF (sin filtro de score — incluye ruido)";
    const txt =
      `${SEP}\n` +
...
export default function BacktestLaboratory() {
  ...
  // CAMBIO Toggle Score Filter (21/05/26): default true (= comportamiento actual).
  const [applyScoreFilter, setApplyScoreFilter] = useState(true);

  // Cuando cambia el toggle, limpiar resultados anteriores para evitar comparaciones inválidas
  // entre runs con filter distinto.
  useEffect(() => {
    setSection2(null);
    setSection3(null);
    setSection4(null);
  ...
  return (
    <ResumenContext.Provider value={{
      ...
      applyScoreFilter, setApplyScoreFilter,
    }}>
      <div style={{ background: T.bg, minHeight: "100dvh", fontFamily: SANS, color: T.text }}>
        <div style={{ padding: "8px 12px calc(8px + env(safe-area-inset-bottom)) 12px", maxWidth: 1200, margin: "0 auto" }}>
          {/* Banner: backtest legacy desactualizado (decisión 03/06/26).
              Recordatorio visible para no usar los números de este
              laboratorio para tomar decisiones hasta que se rehaga
              en proyecto aparte. Ver TP3_knowledge.txt → OUT OF SCOPE. */}
          <div style={{
            background: T.warnBg,
            ...
            <div style={{
              ...
              ⚠ Backtest desactualizado
            </div>
            <div style={{ marginBottom: 4 }}>
              Este laboratorio NO replica el motor del terminal en vivo. Le faltan: gate de noticias, gate R:R y score ajustado por liquidez. Los resultados son inconsistentes — NO usar para decisiones operativas.
            </div>
            <div style={{ color: T.muted, fontSize: 12 }}>
              Pendiente: re-hacer en proyecto aparte (bloqueante para Fase 2).
            </div>
          </div>
          {/* Toggle Score Filter — arriba de todo. Aplica a las 4 secciones. */}
          <ScoreFilterToggle />
