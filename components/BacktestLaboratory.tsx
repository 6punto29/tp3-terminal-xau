...
async function fetchCandles(tf: string, onProgress?: (msg: string) => void): Promise<Candle[]> {
...
async function runBacktest(cfg: BacktestConfig, onProgress?: (msg: string) => void): Promise<BacktestResult> {
  const [htfCandles, mtfCandles] = await Promise.all([
    fetchCandles(cfg.htf, onProgress),
    fetchCandles(cfg.mtf, onProgress),
  ]);
...
function CopyRRBtn({ best }: { best: RRRow }) {
  ...
  const copy = () => {
    ...
    const txt = [
      ...
      `Óptimo: ${best.label}`,
      `SL ${((best.slPct??0)*100).toFixed(2)}% · TP ${((best.tpPct??0)*100).toFixed(2)}% · R:R ${(best.rr??0).toFixed(2)}:1`,
      `Señales: ${best.total} · WR: ${best.wr}% · EV: +${best.ev.toFixed(2)}R`,
      `Avg Win: +${best.avgW.toFixed(2)}% · Avg Loss: ${best.avgL.toFixed(2)}%`,
      ``,
      `→ Configura en terminal: SL ${((best.slPct??0)*100).toFixed(2)}% · TP ${((best.tpPct??0)*100).toFixed(2)}%`,
    ].join("\n");
    navigator.clipboard.writeText(txt).then(() => {
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
...
function Section1() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const res = await runBacktest(cfg, setStat);
      setResult(res);
      const s = res.summary;
      setStat(
          ...
          {best && (
            <div style={{
              ...
              <div style={{fontFamily:MONO,fontSize:13,color:T.text,marginBottom:3}}>
                SL → <strong>{((best.slPct??0)*100).toFixed(2)}%</strong>
                {"   "}·{"   "}
                TP → <strong>{((best.tpPct??0)*100).toFixed(2)}%</strong>
              </div>
