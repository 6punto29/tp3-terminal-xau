...
async function runBacktest(cfg: BacktestConfig, signal?: AbortSignal, onProgress?: (msg: string) => void): Promise<BacktestResult> {
  ...
  const [htfCandles, mtfCandles, m15Extra, ltfCandles, h4Extra] = await Promise.all([
    fetchCandles(cfg.htf, signal, onProgress),
    fetchCandles(cfg.mtf, signal, onProgress),
    needM15 ? fetchCandles("15m", signal, onProgress) : Promise.resolve([] as Candle[]),
    fetchCandles("5m",  signal, onProgress),
    needH4  ? fetchCandles("4h",  signal, onProgress) : Promise.resolve([] as Candle[]),
  ]);
