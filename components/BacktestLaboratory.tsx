...
function Section1() {
  ...
  const [structureF,setStruct] = useState(false);
  ...
  }, [htf, hold, sl, tp, sess, ema200F, structureF]);
  ...
  return (
    <BtCard>
      ...
      <div style={{
        ...
        <span style={{ width: 8 }} />
        <SelBtn active={sess}      onClick={() => setSess(!sess)}         accentColor={T.wait}>LDN/NY</SelBtn>
        <SelBtn active={ema200F}   onClick={() => setEma200(!ema200F)}   accentColor={T.gold}>Dir EMA200</SelBtn>
        <SelBtn active={structureF} onClick={() => setStruct(!structureF)} accentColor={T.accent}>Estructura</SelBtn>
      </div>
  ...
  return (
    <BtCard>
      ...
      {rows && (
        <ResultTable
          rows={rows}
          footer={`Ordenado por EV/trade · SL ${(sl * 100).toFixed(sl * 100 % 1 === 0 ? 0 : 2)}% · TP ${(tp * 100).toFixed(tp * 100 % 1 === 0 ? 0 : 2)}% · spread $0.35 · datos reales Binance Futures`}
        />
      )}
    </BtCard>
  );
    ...
    <BtCard>
      ...
      {rows && (
        <ResultTable
          rows={rows}
          showSLTP
          footer={`Top 20 de ${RR_TEST_CONFIGS.length * RR_COMBOS.length} combinaciones · EMA200 activo · spread $0.35 · datos reales Binance Futures XAUUSDT`}
        />
      )}
    </BtCard>
  );
