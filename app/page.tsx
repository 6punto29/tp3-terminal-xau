...
export default function HomePage() {
  ...
  // B.10: handler del botón 🔔 del topbar.
  ...
  // · granted → sin acción (en B.11 se agregará toggle on/off local con notifEnabled)
  const handleNotifClick = useCallback(() => {
    ...
    // granted → sin acción por ahora (B.11)
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh",
      ...
      <div style={{
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          {/* B.10: botón de notificaciones — visible en desktop Y mobile.
              · 🔔 verde cuando notifPerm === "granted"
              · 🔕 rojo cuando notifPerm === "default" o "denied"
              El comportamiento al tap se maneja en handleNotifClick. */}
          <button
            onClick={handleNotifClick}
            title={
              notifPerm === "granted" ? "Notificaciones activas" :
              notifPerm === "denied"  ? "Notificaciones bloqueadas en el navegador" :
                                        "Activar notificaciones"
            }
            style={{
              background: notifPerm === "granted"
                ? "color-mix(in srgb, var(--tp3-up) 10%, transparent)"
                : "color-mix(in srgb, var(--tp3-down) 10%, transparent)",
              border: notifPerm === "granted"
                ? "1px solid color-mix(in srgb, var(--tp3-up) 50%, transparent)"
                : "1px solid color-mix(in srgb, var(--tp3-down) 50%, transparent)",
              borderRadius: 6, padding: "4px 8px", cursor: "pointer",
