...
export default function HomePage() {
  ...
  // Detecta si la app corre como PWA standalone (instalada en home screen).
  // En PWA standalone iOS, height:100dvh tiene un bug: window.innerHeight crece
  // durante el scroll, desplazando el bottom nav. Como NO hay toolbar en PWA,
  // usamos 100vh + position:fixed (viewport estable). Default neutro (false)
  // para evitar mismatch de hidratación; se resuelve en useEffect client-side.
  ...
  // ── Detección PWA standalone (para el fix del viewport en iOS) ──
  useEffect(() => {
    const mq = window.matchMedia("(display-mode: standalone)");
    // iOS Safari también expone navigator.standalone (no estándar pero fiable)
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column",
      // PWA standalone iOS: 100vh + position:fixed mantiene el viewport estable
      // (window.innerHeight no crece durante el scroll → el bottom nav no se mueve).
      // Safari browser y desktop: 100dvh normal (se adapta a la toolbar dinámica).
      ...(isStandalone
        ? { position:"fixed" as const, top:0, left:0, right:0, bottom:0, height:"100vh" }
        : { height:"100dvh" }),
