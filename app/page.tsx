...
export default function HomePage() {
  ...
  // ── Viewport medido en JS (fix PWA iOS standalone, 07/06/26 noche) ────────
  // Complemento del fix Safari (position:fixed; inset:0 → position:fixed; top/left/right
  // + height medida). En PWA standalone iOS, ni 100dvh ni inset:0 reportan el
  // viewport visible REAL — el wrapper queda con altura incorrecta y el contenido
  // se renderiza más alto que la pantalla, sin posibilidad de scroll interno.
  //
  // visualViewport.height es la API oficial para esto: devuelve la altura del
  // viewport "visualmente visible" considerando barra URL (Safari), teclado
  // (cuando aparece), y el modo de presentación (standalone vs browser).
  // Funciona en iOS 13+ y Android Chrome moderno. Fallback a window.innerHeight
  // y, durante SSR/primer paint sin hidratar, fallback a "100dvh" inline.
  ...
  if (!userId) return (
    <div style={{ background:"var(--tp3-bg,#0B0D11)", minHeight:"100dvh",
  ...
  return (
    // Fix viewport iOS Safari + PWA standalone (07/06/26 noche, handoff v18 post-cierre):
    //
    // ITERACIÓN 1 (Safari): cambio inicial de height:"100dvh" + overflow:"hidden"
    // a position:"fixed" + inset:0. Resolvió Safari iOS browser (la barra URL
    // se ajusta dinámicamente al scroll y el wrapper se ata al viewport visible).
    //
    // ITERACIÓN 2 (PWA standalone): inset:0 NO funcionó en modo PWA standalone iOS
    // — el contenido quedaba renderizado más alto que la pantalla y sin posibilidad
    // de scroll interno. SOLUCIÓN: en lugar de inset:0, usar position:"fixed" con
    // top/left/right explícitos + ALTURA MEDIDA con visualViewport API (state `vh`).
    // Esto evita depender de cómo el browser interpreta inset:0 en cada modo.
    //
    // Cadena interna de alturas intacta (flex:1 → tp3-root height:100% → tp3-sidebar
    // height:100% overflowY:auto). Auditado: ningún ancestro tiene
    // transform/filter/perspective/will-change, así que el modal y el bottom nav
    // siguen posicionándose respecto al viewport. Desktop idéntico a antes.
    <div style={{ display:"flex", flexDirection:"column",
      position:"fixed", top:0, left:0, right:0,
      height: vh !== null ? `${vh}px` : "100dvh",
      ...
      <div style={{
        ...
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          ...
          {/* Botón de alertas — SVG monocromo Lucide-style:
              · Activa (notifPerm granted + notifEnabled): campana llena, color verde (var --tp3-up)
                → semántica universal: verde = activo/on. En modo claro --tp3-gold se veía
                  rojizo/anaranjado y comunicaba lo opuesto (alerta/error).
