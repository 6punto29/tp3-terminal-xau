...
export function precompute(candles: Candle[]): PrecomputedIndicators {
   ...
   *
   * Tracking de estado:
   * - Una FVG alcista se invalida (null) cuando el precio cierra POR DEBAJO de su bot.
   * - Una FVG bajista se invalida cuando el precio cierra POR ENCIMA de su top.
   * - Mientras el precio está DENTRO de la zona, la FVG sigue activa — ese es el punto de entrada.
   *
  ...
  function runFVG(): { bull: FVGZone | null; bear: FVGZone | null }[] {
    ...
    for (let i = 2; i < N; i++) {
      // Detectar nueva FVG alcista
      if (candles[i].l > candles[i - 2].h) {
        activeBull = { top: candles[i].l, bot: candles[i - 2].h, index: i };
      }
      // Detectar nueva FVG bajista
      if (candles[i].h < candles[i - 2].l) {
        activeBear = { top: candles[i - 2].l, bot: candles[i].h, index: i };
      }

      // Invalidar FVG alcista si precio cierra por debajo de su base
      if (activeBull && candles[i].c < activeBull.bot) {
        activeBull = null;
      }
      // Invalidar FVG bajista si precio cierra por encima de su techo
      if (activeBear && candles[i].c > activeBear.top) {
        activeBear = null;
      }
