...
export function getPDHL(d1Candles: Candle[]): { pdh: number | null; pdl: number | null; pdc: number | null } {
  if (d1Candles.length < 1) return { pdh: null, pdl: null, pdc: null };
...
export function precompute(candles: Candle[]): PrecomputedIndicators {
  ...
  function runRSI(p: number): (number | null)[] {
    ...
    for (let i = p; i < N; i++) {
      ...
      const ag = g / p, al = l / p;
      // Fix #9 (v4): precio plano (sin ganancias ni pérdidas) → RSI neutral 50.
      // Antes devolvía 100 con precio plano (al=0 && ag=0), simulando sobrecompra
      // extrema y disparando señal bearish falsa. Misma lógica que rsiCalc().
      if (ag === 0 && al === 0) {
        out[i] = 50;
      } else if (al === 0) {
        out[i] = 100;
