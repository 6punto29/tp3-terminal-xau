...
/** HTF signal scoring — RSI14 con umbrales 25/75 (gold tiende a mantenerse en extremos) */
export function htfSignalAt(ind: PrecomputedIndicators, i: number): HTFResult {
  ...
  let up = 0, dn = 0;
  if (r != null && b) {
    if      (r < 25 && price < b.l) up += 2;   // RSI oversold + bajo BB → fuerte UP
    else if (r > 75 && price > b.u) dn += 2;   // RSI overbought + encima BB → fuerte DN
    else if (r < 30) up++;
    else if (r > 70) dn++;
  } else if (r != null) {
...
/** MTF/LTF confirmation signal — RSI9 con umbrales 30/70 estándar */
