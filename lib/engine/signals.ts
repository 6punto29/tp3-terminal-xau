...
export function getStructureLevels(
  ...
  type Candidate = { price: number; source: "session" | "structure" };
  ...
  // DEBUG TEMPORAL (handoff v5): log compacto para validar Estrategia 2 en vivo.
  // 1 línea por llamada con todos los candidatos y sus ratios. Quitar tras validar.
  const dbg = typeof window !== "undefined";
  const dbgPrefix = `[TP3] dir=${direction} price=${price.toFixed(2)} slDist=${slDist.toFixed(2)} minR=${minRatio} maxR=${maxRR} cands=${candidates.length}`;
  const dbgCands: string[] = [];
  ...
  for (const cand of candidates) {
    ...
    if (ratio < minRatio) {
      dbgCands.push(`${cand.source}@${cand.price.toFixed(0)}/r${ratio.toFixed(2)}/SKIP-close`);
      continue;
    }
    if (ratio > maxRR) {
      dbgCands.push(`${cand.source}@${cand.price.toFixed(0)}/r${ratio.toFixed(2)}/NULL-far`);
      if (dbg) console.log(`${dbgPrefix} → ${dbgCands.join(" ")}`);
      return null;   // muy lejos → vacío estructural, no entrar
    }

    // Candidato válido en rango [minRatio, maxRR]
    dbgCands.push(`${cand.source}@${cand.price.toFixed(0)}/r${ratio.toFixed(2)}/MATCH`);
    if (dbg) console.log(`${dbgPrefix} → ${dbgCands.join(" ")}`);
  ...
  if (dbg) console.log(`${dbgPrefix} → ${dbgCands.join(" ")} ALL-skip-NULL`);
  return null;
}
