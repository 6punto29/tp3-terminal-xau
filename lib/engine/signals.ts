...
/**
 * Calcula SL/TP estructurales para MT5.
 *
 * ─── ESTRATEGIA 2 (handoff v5 · 27/05/26 noche) ──────────────────────────────
 * El motor recorre TODOS los candidatos estructurales (PDH/PDL + swing highs/lows
 * del 1H) en orden de cercanía al precio, y devuelve el PRIMERO que cumpla R:R
 * en el rango [minRatio, maxRR]. Sin TPs inventados, sin rescates artificiales.
 *
 * Reglas de selección de TP:
 *   - ratio < minRatio  → candidato muy cerca, paso al siguiente
 *   - ratio > maxRR     → vacío estructural, NO entrar (return null)
 *   - ratio en rango    → ese es el TP
 * Si ningún candidato cae en el rango → return null (sin TP válido).
 *
 * SL: último swing high/low en la dirección opuesta al precio, capeado a
 * slCapPct. Sin cambios respecto a versión previa.
 *
 * Parámetros:
 * - slCapPct:    tope máximo de SL en % desde entry. Default 0.75%.
 * - tpTargetPct: LEGACY — mantenido por compatibilidad con detectSignals (call
 *                site del backtest). NO se usa en la nueva lógica.
 * - minRatio:    R:R mínimo válido. Default 2 (= gate minRR del terminal).
 * - maxRR:       R:R máximo válido. Default 5. Más allá = vacío estructural.
 *
 * Return: StructureLevels si encuentra TP válido en el rango, null si no.
 * ─────────────────────────────────────────────────────────────────────────────
 */
export function getStructureLevels(
  ...
  direction:   "UP" | "DOWN",
  price:       number,
  session:     SessionLevels = { pdh: null, pdl: null, pdc: null },
  slCapPct:    number = 0.75,
  tpTargetPct: number = 3,    // legacy, sin uso en la nueva lógica
  minRatio:    number = 2,    // R:R mínimo (antes 0.5; subido al gate minRR=2)
  maxRR:       number = 5     // NUEVO: R:R máximo válido
): StructureLevels | null {
  // Nota: tpTargetPct se mantiene en la firma por compatibilidad con
  // detectSignals (backtest, línea ~657). La nueva lógica de TP estructural
  // buscado no lo usa. Si el backtest se reabre, evaluar si quitarlo.
  void tpTargetPct;
  ...
  // ── Cálculo del SL (sin cambios vs versión previa) ──
  let slPrice: number;
  ...
  if (slDist <= 0) return null; // edge case: precio = SL, sin sentido operar

  // ── Construir lista de candidatos de TP ──
  type Candidate = { price: number; source: "session" | "structure" | "pivot" };
  const candidates: Candidate[] = [];

  if (direction === "UP") {
    // LONG: candidatos ARRIBA del precio (PDH + swing highs del 1H)
    if (session.pdh != null && session.pdh > price) {
      candidates.push({ price: session.pdh - buffer, source: "session" });
    }
    for (let j = i - 1; j >= 0; j--) {
      if (swingH[j] != null && swingH[j]! > price) {
        candidates.push({ price: swingH[j]! + buffer, source: "structure" });
      }
    }
  } else {
    // SHORT: candidatos ABAJO del precio (PDL + swing lows del 1H)
    if (session.pdl != null && session.pdl < price) {
      candidates.push({ price: session.pdl + buffer, source: "session" });
    }
    for (let j = i - 1; j >= 0; j--) {
      if (swingL[j] != null && swingL[j]! < price) {
        candidates.push({ price: swingL[j]! - buffer, source: "structure" });
      }
    }
  }

  // ── Pivotes clásicos del día anterior (handoff v5 · 27/05/26) ──
  // Niveles institucionales reales. Se agregan como candidatos adicionales para
  // cubrir casos donde el precio ya rompió PDL/PDH y no hay swing del 1H abajo/arriba.
  // Fórmula clásica: P=(H+L+C)/3, R/S derivados del rango del día anterior.
  if (session.pdh != null && session.pdl != null && session.pdc != null) {
    const P     = (session.pdh + session.pdl + session.pdc) / 3;
    const rango = session.pdh - session.pdl;

    if (direction === "UP") {
      // LONG: resistencias arriba del precio
      const R1 = 2 * P - session.pdl;
      const R2 = P + rango;
      const R3 = session.pdh + 2 * (P - session.pdl);
      for (const r of [R1, R2, R3]) {
        if (r > price) candidates.push({ price: r - buffer, source: "pivot" });
      }
    } else {
      // SHORT: soportes abajo del precio
      const S1 = 2 * P - session.pdh;
      const S2 = P - rango;
      const S3 = session.pdl - 2 * (session.pdh - P);
      for (const s of [S1, S2, S3]) {
        if (s < price) candidates.push({ price: s + buffer, source: "pivot" });
      }
    }
  }

  // Ordenar por cercanía al precio (más cerca primero)
  candidates.sort((a, b) => Math.abs(a.price - price) - Math.abs(b.price - price));

  // ── DEBUG [TP3] · solo en navegador. Borrable sin afectar la lógica. ──
  const dbg = typeof window !== "undefined";
  const trace: string[] = [];

  // ── Recorrer candidatos y devolver el primero válido ──
  for (const cand of candidates) {
    const tpDist = Math.abs(cand.price - price);
    const ratio  = tpDist / slDist;

    if (ratio < minRatio) {              // muy cerca → siguiente candidato
      if (dbg) trace.push(`${cand.source}@${cand.price.toFixed(2)}/r${ratio.toFixed(2)}/SKIP-close`);
      continue;
    }
    if (ratio > maxRR) {                 // muy lejos → vacío estructural, no entrar
      if (dbg) {
        trace.push(`${cand.source}@${cand.price.toFixed(2)}/r${ratio.toFixed(2)}/NULL-far`);
        console.log(`[TP3] dir=${direction} price=${price.toFixed(2)} slDist=${slDist.toFixed(2)} minR=${minRatio} maxR=${maxRR} cands=${candidates.length} → ${trace.join(" | ")}`);
      }
      return null;
    }

    // Candidato válido en rango [minRatio, maxRR]
    const slPct = slDist / price * 100;
    const tpPct = tpDist / price * 100;

    if (dbg) {
      trace.push(`${cand.source}@${cand.price.toFixed(2)}/r${ratio.toFixed(2)}/MATCH`);
      console.log(`[TP3] dir=${direction} price=${price.toFixed(2)} slDist=${slDist.toFixed(2)} minR=${minRatio} maxR=${maxRR} cands=${candidates.length} → ${trace.join(" | ")}`);
    }
    ...
    return {
      ...
      tpPrice:    Math.round(cand.price * 100) / 100,
      atrValue,
      slPct:      Math.round(slPct * 100) / 100,
      tpPct:      Math.round(tpPct * 100) / 100,
      tpSource:   cand.source,
    };
  }

  // Ningún candidato cae en el rango [minRatio, maxRR] → no entrar
  if (dbg) {
    console.log(`[TP3] dir=${direction} price=${price.toFixed(2)} slDist=${slDist.toFixed(2)} minR=${minRatio} maxR=${maxRR} cands=${candidates.length} → ${trace.length ? trace.join(" | ") : "(sin candidatos en dirección)"} → ALL-skip-NULL`);
  }
  return null;
}
...
//   -3 sobre-extensión: precio a >4.75% de la EMA200, en la dirección del trade (13/06/26)
//
...
export function calcSignalScore(
  ...
  // ── 5º filtro — SOBRE-EXTENSIÓN (13/06/26) ──────────────────────────────────
  // No perseguir entradas muy estiradas de la EMA200. Si el precio ya se despegó
  // demasiado de su media de largo plazo, la entrada llega a destiempo (zona de
  // agotamiento). Castigo BLANDO -3 al score, no bloqueo duro: un setup excepcional
  // (score muy alto) sobrevive y entra; uno marginal (en el umbral) cae por debajo
  // y se descarta. Así la convicción rompe el empate.
  //
  // Umbral 4.75% (conservador), validado con 13 señales reales (reversa en V 08-12/06):
  // fórmula estándar (precio-EMA200)/EMA200 → las 3 GANADORAS a ≤4.15%; las 10
  // PERDEDORAS a ≥4.62% (hueco limpio 4.15→4.62). A 4.75%: mantiene las 3 ganadoras
  // (0.60% de margen) y bloquea 8 de 10 perdedoras; cuela solo 2 pegadas al hueco.
  //
  // Direccional — solo penaliza cuando el precio está estirado EN la dirección
  // del trade (no penaliza shortear un rebote ni comprar un retroceso):
  //   SHORT estirado = precio MUY por DEBAJO de la EMA200
  //   LONG  estirado = precio MUY por ENCIMA de la EMA200
  //
  // Riesgo conocido (IG/Schwab): en tendencia fuerte la sobre-extensión PERSISTE
  // (lo sobrevendido puede seguir cayendo). Por eso es castigo blando y no gate:
  // deja pasar la continuación de tendencia de alta convicción. A vigilar en demo.
  if (ema200 != null && price > 0) {
    const overextPct = Math.abs(price - ema200) / ema200;
    if (htf === "DOWN" && price < ema200 && overextPct > 0.0475) s -= 3;
    if (htf === "UP"   && price > ema200 && overextPct > 0.0475) s -= 3;
  }
...
export function detectSignals(
  ...
  const dailyHL = new Map<string, { h: number; l: number; c: number }>();
  const orderedDays: string[] = [];
  for (const c of htfCandles) {
    const k = dayKey(c.t);
    const cur = dailyHL.get(k);
    if (!cur) {
      dailyHL.set(k, { h: c.h, l: c.l, c: c.c });
      orderedDays.push(k);
    } else {
      if (c.h > cur.h) cur.h = c.h;
      if (c.l < cur.l) cur.l = c.l;
      cur.c = c.c; // se actualiza siempre → queda el cierre de la última vela del día
    }
  }
  // Para cada día, guardar referencia al día anterior (el "PDH/PDL/PDC" de ese día)
  const prevDayHL = new Map<string, { pdh: number; pdl: number; pdc: number } | null>();
  for (let d = 0; d < orderedDays.length; d++) {
    if (d === 0) prevDayHL.set(orderedDays[d], null);
    else {
      const prev = dailyHL.get(orderedDays[d - 1])!;
      prevDayHL.set(orderedDays[d], { pdh: prev.h, pdl: prev.l, pdc: prev.c });
    }
  }
  ...
  for (let i = 50; i < htfCandles.length - holdCandles; i++) {
    ...
    const session: SessionLevels = sessionPrev
      ? { pdh: sessionPrev.pdh, pdl: sessionPrev.pdl, pdc: sessionPrev.pdc }
    ...
    // Estrategia 2 (handoff v5): getStructureLevels puede devolver null si no
    // hay TP estructural en el rango [minRatio, maxRR]. En ese caso no se
    // genera señal — coherente con el comportamiento del terminal.
    if (!levels) continue;
...
export function getLiveVerdict(
  ...
  // R:R mínimo del gate. Subido de 1.5 a 2 el 27/05/26 (handoff v4, pendiente #4).
  // Pérdidas no mayores que ganancias — principio R:R 2:1 del proyecto.
  minRR:       number = 2
): LiveVerdict {
  ...
  // FVG direccional (fix 07/06/2026): solo cuenta si confirma la dirección
  // del setup (hSig). Alineado con isInFVG (diseñada direccional según su
  // docstring), con el resto del motor (EMA200/estructura/4H direccionales),
  // con el backtest legacy (línea ~677 que lo calcula direccional) y con
  // la literatura SMC unánime (7 fuentes coincidentes documentadas en el
  // handoff v17). Bug previo: usar UP||DOWN volvía el cálculo agnóstico
  // → sumaba +1 al score incluso cuando el FVG iba en contra del trade.
  const fvgActive = livePrice > 0 && hSig !== "WAIT" &&
    isInFVG(htfInd, htfIdx, hSig, livePrice);
