...
export function htfSignalAt(ind: PrecomputedIndicators, i: number): HTFResult {
  ...
  // TEST EXPERIMENTAL (21/05/26): diferencia mínima REVERTIDA a 1 (antes 2).
  // Hipótesis: el cambio a 2 (Bug 3.2) descartó las señales 3-2 que en backtest
  // 15/05 representaban ~26 señales/mes con WR ~69%. Revertir a 1 debería volver
  // a dar ~39 señales/mes (config validada original).
  // Si funciona → mantener en 1. Si no → revertir a 2 (cambio quirúrgico).
  const sig: SignalDirection =
    up - dn >= 1 ? "UP" : dn - up >= 1 ? "DOWN" : "WAIT";
  return { sig, em200 };
}
...
export function mtfSignalAt(ind: PrecomputedIndicators, i: number): SignalDirection {
  ...
  // TEST EXPERIMENTAL (21/05/26): diferencia mínima REVERTIDA a 1 (antes 2).
  // Misma razón que en htfSignalAt — recuperar señales descartadas por Bug 3.2.
  return up - dn >= 1 ? "UP" : dn - up >= 1 ? "DOWN" : "WAIT";
}
...
export function detectSignals(
  ...
  cfg: {
    ...
    // aunque se pasen los timeframes extra (m15/ltf/h4). Útil para A/B testing.
    // Default true mantiene comportamiento post-Fix #2 (backtest = live).
    // Si false: backtest acepta TODAS las señales que pasen HTF+MTF (= comportamiento pre-Fix #2).
  ...
  // CAMBIO Toggle (21/05/26): respetamos cfg.applyScoreFilter (default true)
  // Si false → no calculamos indicadores extra ni aplicamos score filter (ahorra CPU).
  const applyScoreFilter = cfg.applyScoreFilter !== false;  // default true
...
export function getLiveVerdict(
  ...
  slCapPct:    number = 0.75,
  // CAMBIO Fix #3 (Auditoría 20/05/26): default 3 (antes 2.5).
  // Alinea con getStructureLevels, detectSignals y config validada.
  tpTargetPct: number = 3
): LiveVerdict {
