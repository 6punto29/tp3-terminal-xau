...
export function mtfSignalAt(ind: PrecomputedIndicators, i: number): SignalDirection {
  ...
  // REVERSIÓN (21/05/26): diferencia mínima de vuelta a 2 (valor validado original).
  // Misma razón que en htfSignalAt — bajar a 1 descoordinaba los timeframes.
...
export function getLiveVerdict(
  ...
  const gates = {
    sesion:  sessionTag === "LDN" || sessionTag === "NY",
  ...
  // ── CAPA 3 — SCORE DE CALIDAD ──
  // Usa calcSignalScore y getScoreThreshold tal cual existen hoy.
  // La recalibración (umbral, normalización) se hará con backtest — ver handoff.
  const score       = calcSignalScore(
  ...
  else if (score < scoreUmbral)     detenidoEn = "capa3";
