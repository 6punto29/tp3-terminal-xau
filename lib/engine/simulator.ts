...
//
// Cambios v6 (25/05/26 — fix off-by-one del Hold):
// · simulateSignals ahora respeta "Hold N" = EXACTAMENTE N velas.
//   ANTES: el loop revisaba N+1 velas y el cierre por tiempo tomaba la última
//   vela del array htfFuture (N+2), dándole al trade más tiempo del configurado.
//   AHORA: el loop revisa N velas (índices 0..N-1) y el cierre por tiempo usa
//   el close de la N-ésima vela. "Hold 6" cierra a las 6 velas, no a las 7-8.
//
 ...
 *
 * Definición del Hold (v6): un trade vive como máximo `holdCandles` velas.
 * Se revisa SL/TP en esas N velas (índices 0..N-1 de htfFuture). Si en esas
 * N velas no se tocó ni SL ni TP, el trade se cierra por tiempo al close de
 * la N-ésima vela. "Hold N" = exactamente N velas.
 */
export function simulateSignals(
  ...
  return signals.map((s) => {
    ...
    // El trade vive como máximo `holdCandles` velas. Se revisa SL/TP en esas
    // velas (índices 0..holdCandles-1). Si htfFuture tiene menos velas que
    // holdCandles (final del dataset), se revisan solo las disponibles.
    const limit = Math.min(s.htfFuture.length, holdCandles);
    ...
    // Cierre por tiempo: si no tocó SL ni TP dentro del Hold, se cierra al
    // close de la ÚLTIMA vela del Hold (índice limit-1) — nunca más allá.
    if (!hitTP && !hitSL) {
      exitP = limit > 0
        ? s.htfFuture[limit - 1].c
        : s.entry;
      won =
